/-
  ...
  So the scheduler is CORRECT-BY-CONSTRUCTION: `schedule_refines` is PROVEN, `sorry`-free
  (`#print axioms` = `[propext, Classical.choice, Quot.sound]`, no `sorryAx`).

  ★ THE FRONTIER IS CLOSED. Earlier drafts of this header listed a residual `sorry` frontier
  (F1 `nodes_stepOk`, F2 `exitAlt_stepOk`, F3 `arrange_stepOk`, F4a `final_readout_main`), all
  "downstream of ONE invariant (the home/demand liveness map)". That invariant was pinned and all
  four are now discharged as ordinary lemmas:
    (F1) `nodes_stepOk`   — per-node emission preserves `Inv` (the home/demand liveness invariant
         threaded across the node fold + the denotation link `n.op.sem (n.ins.map dRef) =
         blockEnv … n.id` feeding `emitCall_stepOk.hout`).
    (F2) `exitAlt_stepOk` — exit-alt deposit preserves `Inv`.
    (F3) `arrange_stepOk` / `emitExitMain_stepOk` — final main arrange (matchPrefix + build +
         stale-tail delete) preserves `Inv`, off the R4 liveness fact `∀ r ∈ b.exit, RefHasHome …`.
    (F4a) `final_readout_main` — the final MAIN stack reads out to the exit DAG layout.
    (F4b) `final_readout_alt`  — the ALT stack reads out to the exit-alt layout.
  The ONE remaining gap is external: `schedule_refines` is over the abstract `Opt.run`; grounding it
  in the real `stepInstr` is the open Adapter bridge (`LeanBCH.Opt.Adapter`).
...
theorem emitExitMain_S {α} [DecidableEq α] (b : Block α) (ms : ModelState α)
    (_hLS : matchPrefixAux ms.S 0 b.exit ≤ ms.S.length)
    (_hLe : matchPrefixAux ms.S 0 b.exit ≤ b.exit.length) :
