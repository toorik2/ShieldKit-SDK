...
// ---- opcode / VM constants ---------------------------------------------------
// (define/invoke framing + id encoding now come from the dialect via emitInvokeOps/emitDefineOps)
...
function applyFactorings(d, ctx, chosen) {
  ...
  for (const [si, reps] of bySi) {
    ...
    for (const r of reps) arr.splice(r.i, r.L, ...emitInvokeOps(r.id));
  }
  ...
  for (const c of chosen) {
    for (const o of emitDefineOps(c.id, c.contentBytes)) out.push(o);
  }
...
function runGates(orig, factored, d, applied, ctx, { quiet = false } = {}) {
  ...
  for (const c of chosen) {
    ...
    const invokeSite = serialize(emitInvokeOps(c.id));
