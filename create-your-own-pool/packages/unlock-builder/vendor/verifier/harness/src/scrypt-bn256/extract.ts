// Extract the real sCrypt BSV Groth16 verifier opcodes from mainnet (BN256).
//
//   parent  tx 320b...725f  vout[0]  -> the ~11.7 MB verifier LOCKING script
//   spending tx 24e8...bf24  vin[0]   -> the 510 B proof/witness UNLOCKING script
//
// We decode both transactions, pull the two scripts, disassemble them to a
// readable opcode listing, and summarise which opcodes the program uses. This is
// sCrypt's first (Jul 2022) Groth16 verifier, over BN256 / alt_bn128 / BN254 --
// the same curve as groth16_contract's BN256.cash -- so its opcode and
// field-element profile lines up directly with that target.
...
const parent = decode('parent', 'data/scrypt-bn256/parent-tx.hex');
const spending = decode('spending', 'data/scrypt-bn256/spending-tx.hex');
...
console.log('parent  txid (locking source):', binToHex(parent.outputs[0]!.lockingBytecode).slice(0, 0) || '320b...725f');
console.log('spending txid (unlocking)    :', '24e8...bf24');

report(
  'BSV Groth16 LOCKING script (the verifier)',
  lockingBytecode,
  'data/scrypt-bn256/groth16-locking.asm',
);
report(
  'BSV Groth16 UNLOCKING script (proof + witness)',
  unlockingBytecode,
  'data/scrypt-bn256/groth16-unlocking.asm',
);
