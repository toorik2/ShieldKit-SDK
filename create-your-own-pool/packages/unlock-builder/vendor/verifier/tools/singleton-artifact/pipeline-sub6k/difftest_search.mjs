// difftest.mjs -- per-subroutine DIFFERENTIAL TEST of the scheduler.
// For each of the 52 OP_DEFINE bodies: build the optimized body (per-block min of
// scheduled/original), then run BOTH the original and optimized subroutine in isolation
// on random Fp inputs on the loosened BCH-2026 VM and require BIT-IDENTICAL output stacks.
// Reports per-subroutine: bytes orig->opt, blocks scheduled, and diff-test verdict.
...
// cache arity (probe is ~38s)
const arityPath = join(here, 'arity.json');
let arity;
if (existsSync(arityPath)) { arity = JSON.parse(readFileSync(arityPath, 'utf8')); }
else { console.time('probeArity'); arity = probeArity(d); console.timeEnd('probeArity'); writeFileSync(arityPath, JSON.stringify(arity)); }

const P = 21888242871839275222246405745257275088696311157297823662689037894645226208583n;
// deterministic random Fp
...
for (const id of d.order) {
  ...
  const { bytes: optBytes, chosen } = optimizeItems(items, arity, 'sched', { peephole: true, search: { cap: 20000, trials: 1500 } });
...
console.log(`\nBODIES total: ${totOrig} -> ${totOpt} B  (saved ${totOrig - totOpt} B, ${(100 * (totOrig - totOpt) / totOrig).toFixed(1)}%)`);
