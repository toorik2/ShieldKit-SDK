/-
  ...
  Integrated into the project (`Recompiler.Dag` import) as `Recompiler/Floor.lean`; the counting
  model lives in the nested `Recompiler.Floor` namespace, and `Recompiler.fanout` ties the model's
...
/-! ## Bridging the copy-floor to the value-DAG (`Recompiler.Dag`).
    ...
    (`Recompiler.Scheduler.emitFetch`) serves each consuming edge by exactly one stack primitive:
