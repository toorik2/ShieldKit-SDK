...
| Field | Value |
|-------|--------|
| **Instance ref** | `use-chipnet-demo-pool` (aliases: `chipnet-playground`, `playground`) |
...
| **Descriptor** | [`use-chipnet-demo-pool/instance.json`](../../use-chipnet-demo-pool/instance.json) |
...
Created via `create-your-own-pool` (`init` → bundle → `instance.json` → genesis).  
