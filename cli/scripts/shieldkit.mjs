...
// ---- unified verifier-pool profile routing ----
// --profile <id> selects the verifier-pool design. Absent (or "pf10") = the
// original ShieldKit-Groth beta product surface, unchanged. The pf6 profile is
// the first alternative verifier-pool design (bn254-onetx-pf6-a3-r1, 6 roles,
// 9-input actions) and shares the same command surface, options, and fail-closed
// JSON envelope family.
// ---- the unified --bench flag (flag only; works for EVERY registered pool design) ----
//   shieldkit --profile <id> --bench [--cold-start] [--json]
// Benchmarks the ACTIVE design on two honest axes (Axis A: on-chain verifier
// surface; Axis B: prep time with the prover identity). No new commands.
const BENCH_INDEX = process.argv.indexOf('--bench');
...
if (PROFILE_INDEX >= 0 && (PROFILE_VALUE === undefined || PROFILE_VALUE.startsWith('--'))) {
...
if (PROFILE_VALUE !== undefined && PROFILE_VALUE !== 'pf10' && PROFILE_VALUE !== 'shieldkit-groth-94kb') {
  try {
    ...
    if (e?.code === 'ERR_MODULE_NOT_FOUND' || e?.code === 'MODULE_NOT_FOUND') {
      console.log(JSON.stringify({ ok: false, code: 'UNKNOWN_PROFILE', error: `unknown verifier-pool profile: ${PROFILE_VALUE}` }, null, 2));
      process.exit(2);
    }
    if (e?.name === 'Pf6ProfileError') {
      console.log(JSON.stringify({ ok: false, code: e.code, error: e.message }, null, 2));
      process.exit(e.exitCode ?? 2);
    }
    console.log(JSON.stringify({ ok: false, code: 'PROFILE_INTERNAL', error: String(e?.message ?? e) }, null, 2));
    process.exit(2);
  }
}
