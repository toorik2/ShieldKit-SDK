...
test('packed CLI contains its runtime closure and runs without the checkout', () => {
  ...
  try {
    ...
    const bin = path.join(installed, 'cli/bin/shieldkit.mjs');
    assert.ok(existsSync(path.join(installed, 'cli/front-controller.mjs')));
    assert.ok(existsSync(path.join(installed, 'cli/contracts/errors.mjs')));
