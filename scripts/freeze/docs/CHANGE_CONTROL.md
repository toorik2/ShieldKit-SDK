...
## 2026-07-25d — Product-only repository surface

- Removed `dev/` from the product repository (protocol labs/evidence/research no longer shipped in-tree).
- G0 freeze artifacts live under `scripts/freeze/`.
- Gate process docs live under `scripts/freeze/docs/` (BUILD_PLAN, KILL_GATES, OPEN_QUESTIONS, CHANGE_CONTROL).
- Product docs remain `docs/{CHARTER,PRIVACY,...}` and root README/SECURITY.
- GitHub Actions and `.gitattributes` removed earlier / now.
