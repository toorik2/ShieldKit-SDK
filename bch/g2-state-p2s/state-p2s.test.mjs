...
async function fixtures() {
  ...
  const deposit = { kind: 'deposit', networkId: 2, profileId, instanceId, preState: initial, postState: depositPost, depositSats: DENOMINATION_SATS.toString(), outputNote: note1, noteAppendPath: { siblings: notePathEmpty.map(frToHex) }, outputRecord: Buffer.alloc(OUTPUT_RECORD_BYTES, 1), transactionContextDigest: createHash('sha256').update('deposit-context').digest('hex') };
