...
async function timedNativeProverInstall({ repoDir, sourceNative, destNative, env }) {
  ...
  const modHref = pathToFileURL(
    path.join(repoDir, 'designs/pf10/packages/prove/v2/native-groth16-prover-installation.mjs'),
  ).href;
...
async function measureColdProveFromSandbox(sandboxRepo, liveDataHome) {
  const proveUrl = pathToFileURL(
    path.join(sandboxRepo, 'designs/pf10/bench/pf10-baseline/product-prove.mjs'),
  ).href;
    ...
    if (!existsSync(path.join(liveArt, 'runtime'))
      ...
      if (!native.ok) {
        ...
        const installScript = path.join(
          repoDir,
          'designs/pf10/scripts/install-v2-beta-product-artifacts.mjs',
        );
  ...
  if (args.timeNpmCi) {
    ...
    const rsync = timeCommand(
      ...
      ['-a', '--exclude', 'node_modules', '--exclude', '.git',
        `${root}/designs/pf10/`, `${dest}/designs/pf10/`],
      root,
    );
    if (rsync.status !== 0) {
      steps.push({
