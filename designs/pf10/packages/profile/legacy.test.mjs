...
test('legacy create-pool and chain E2E scripts are outside the PF10 product tree', async () => {
  ...
  assert.equal(
    existsSync(path.join(repositoryRoot, 'designs/pf10/scripts/create-pool.mjs')),
    false,
  );
  assert.equal(
    existsSync(path.join(repositoryRoot, 'designs/pf10/scripts/chain-e2e-chipnet.mjs')),
    false,
  );
  assert.equal(
    existsSync(path.join(repositoryRoot, 'designs/pf10/research/v1-seven-carrier/create-pool.mjs')),
    true,
  );
  assert.equal(
    existsSync(path.join(repositoryRoot, 'designs/pf10/research/v1-seven-carrier/chain-e2e-chipnet.mjs')),
    true,
  );
});
  ...
  await assert.rejects(
    execFileAsync(
      ...
      [
        path.join(
          repositoryRoot,
          'designs/pf10/scripts/shieldkit.mjs',
        ),
        'init',
        '--config',
