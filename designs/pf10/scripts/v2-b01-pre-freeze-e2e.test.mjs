...
function verifyCommand(checkout, bundle) {
  return nodeCommand(
    checkout,
    'designs/pf10/scripts/v2-b01-pre-freeze.mjs',
    ['--verify', bundle],
  );
}
...
test('B-01 real-artifact clean-clone create, verify, and tamper closure', {
  ...
  const q01Result = requireSuccess(nodeCommand(
    checkout,
    'designs/pf10/scripts/v2-q01-commit-bound-evidence.mjs',
  ...
  await t.test('real B-01 source identity prepares the exact beta custody inventory', () => {
    ...
    const result = requireSuccess(nodeCommand(
      checkout,
      'designs/pf10/scripts/v2-beta-single-contributor-ceremony.mjs',
      [
        'prepare',
