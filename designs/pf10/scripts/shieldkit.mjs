 ...
 * The package bin points at the unified CLI. This file remains the PF10
 * lifecycle backend and is invoked through that grammar.
  ...
  if (playgroundMode()) {
    try {
      ...
      failJson(e.code || e.name || 'PLAYGROUND_ERROR', e.message || String(e), 2, {
        ...
        docs: 'designs/pf10/demo/chipnet/README.md',
      });
    }
  }
...
async function cmdTip() {
  try {
    ...
    const poolDir = arg('pool')
      ...
      : path.join(monorepoRoot, 'designs/pf10/demo/chipnet');
    ...
    if (!existsSync(instancePath) || !existsSync(bundleDir)) {
      failJson('POOL_MISSING', 'need designs/pf10/demo/chipnet/ (or --pool) with instance.json + bundle/', 2);
    }
    ...
    failJson('LEGACY_AUTHORITY_MOVED', 'legacy seven-carrier authority helpers moved to designs/pf10/research/v1-seven-carrier/', 2);
    const {
...
async function cmdPlaygroundDoctor() {
  try {
    ...
    const body = {
      ...toolkitIdentity(),
      story: 'ShieldKit creates shielded pools. Demo is designs/pf10/demo/chipnet/; product is designs/pf10/.',
      product: 'designs/pf10/',
...
async function cmdInit() {
  failJson(
    ...
    {
      setupCommand:
        'node designs/pf10/packages/profile/setup/development-cli.mjs --input <attested-v2-setup.json>',
      profileCommand:
        'node designs/pf10/scripts/v2-development-profile.mjs <all pinned artifact arguments>',
...
async function cmdAct(verb) {
  ...
  if (arg('pool') || playgroundFullAct) {
    ...
    const monorepoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../..');
    failJson('LEGACY_POOL_ACT_MOVED', 'legacy seven-carrier pool-act moved to designs/pf10/research/v1-seven-carrier/; use shieldkit pool deposit|withdraw for PF10', 2);
    const script = null;
    const poolDir = arg('pool')
      ? path.resolve(arg('pool'))
      : path.join(monorepoRoot, 'designs/pf10/demo/chipnet');
  ...
  if (!hasPool || !requestPath) {
    failJson(
      ...
      2,
      {
        ...
        feeNote: 'Legacy playground deposit needs a large fee UTXO; transfer/withdraw ~71k. See designs/pf10/demo/chipnet/README.md',
