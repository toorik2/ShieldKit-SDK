...
function assertRealRunProvenance(runRecord, source, machine, root) {
  ...
  const expectedManifest = join(source.sourceRoot, 'designs/pf10/crates/shieldkit-v2-recovery/Cargo.toml');
  const expectedBinary = join(source.sourceRoot, 'designs/pf10/crates/shieldkit-v2-recovery/target/release/q07-lifecycle-verify');
  ...
  for (const [index, expectedArgsForCommand] of [
    [join(source.sourceRoot, 'designs/pf10/scripts/v2-q07-lifecycle-corpus.mjs'), '--output-directory', root],
    [join(source.sourceRoot, 'designs/pf10/scripts/v2-q07-lifecycle-corpus.mjs'), '--verify', corpusPath],
  ].entries()) {
...
function buildCommand(sourceRoot, toolchain) {
  const manifest = join(sourceRoot, 'designs/pf10/crates/shieldkit-v2-recovery/Cargo.toml');
  if (toolchain === null || Array.isArray(toolchain) || typeof toolchain !== 'object'
...
function expectedBinaryPath(sourceRoot) {
  return join(sourceRoot, 'designs/pf10/crates/shieldkit-v2-recovery/target/release/q07-lifecycle-verify');
}
...
function realResumeExecutor() {
  return Object.freeze({
    ...
    async jsVerifier({ sourceRoot, corpusPath }) {
      const script = join(sourceRoot, 'designs/pf10/scripts/v2-q07-lifecycle-corpus.mjs');
...
function assertResumeCommand(command, phase, sourceRoot, corpusPath, testOnly, systemd = null, toolchain = null, buildSnapshot = null) {
  ...
  if (phase === 'js-verifier') {
    const expected = [
      join(sourceRoot, 'designs/pf10/scripts/v2-q07-lifecycle-corpus.mjs'),
      '--verify',
      corpusPath,
    ];
