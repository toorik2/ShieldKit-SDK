...
test('PF10 Libauth evidence is generated before and explicitly bound into the runtime', () => {
  ...
  const libauth = pf10LibauthQualificationArguments(
    '/repository/designs/pf10',
    paths,
  );
  assert.deepEqual(libauth, [
    '/repository/designs/pf10/scripts/v2-pf10-libauth-qualification.mjs',
    '--output', paths.output,
...
test('local verifier provisioning is an attested PF10-only pipeline in dependency order', async () => {
  ...
  assert.equal(
    /(?:v2-legacy-verifier-qualification|\bseven-carrier(?:-|\b)|legacy-seven-carrier)/.test(provisioner),
    false,
    'the legacy seven-carrier topology is not a PF10 deployment dependency',
  );
...
test('CI provisions local verifier artifacts before executing the lane', async () => {
  ...
  assert.equal(
    ...
    'node designs/pf10/scripts/run-domain-tests.mjs --provision-local-verifier-artifacts',
  );
  ...
  assert.equal(
    ...
    'node designs/pf10/scripts/run-domain-tests.mjs --suite local-strict-codec-campaign',
  );
  assert.equal(
    packageJson.scripts['test:v2:campaign:depth4'],
    'node designs/pf10/scripts/run-domain-tests.mjs --suite local-depth4-campaign',
  );
  assert.equal(
    packageJson.scripts['qualification:v2:crash10k'],
    'node designs/pf10/scripts/v2-crash-qualification.mjs',
  );
  assert.equal(
    packageJson.scripts['qualification:v2:reorg-concurrency'],
    'node designs/pf10/scripts/v2-reorg-concurrency-qualification.mjs',
  );
  assert.equal(
    packageJson.scripts['qualification:v2:pf10-libauth'],
    'node designs/pf10/scripts/v2-pf10-libauth-qualification.mjs',
  );
});
