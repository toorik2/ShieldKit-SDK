...
export function snapshotExecutionPaths(snapshotRoot, bundle) {
  ...
  return Object.freeze({
    ...
    recoveryCwd: assertContained(root, join(root, "designs/pf10/crates/shieldkit-v2-recovery"), "Q-04 Rust source"),
    ...
    productionNullifierSource: assertContained(
      ...
      join(
        root,
        "designs/pf10/packages/pool/v2/persistent-indexed-nullifier.mjs",
      ),
    ...
    historyRunner: assertContained(root, join(root, "designs/pf10/scripts/v2-q04-history-runner.mjs"), "Q-04 history runner"),
    evidenceVerifier: assertContained(root, join(root, "designs/pf10/scripts/v2-q04-evidence-verify.mjs"), "Q-04 evidence verifier"),
    rust: layout,
  });
}
...
async function loadSnapshotModules(snapshotRoot) {
  ...
  const [schedule, evidence, depth4] = await Promise.all([
    moduleAt("designs/pf10/packages/pool/v2/qualification/q04-schedule.mjs"),
    moduleAt("designs/pf10/scripts/v2-q04-evidence-verify.mjs"),
    moduleAt("designs/pf10/packages/pool/v2/qualification/depth4-production-state-space.mjs"),
  ]);
...
function sourceArtifacts(bundle, snapshotRoot, definitions) {
  ...
  const sources = Object.freeze({
    ...
    campaign,
    depth4,
    cargoManifest: copyNamed("rust-kat-manifest", "designs/pf10/crates/shieldkit-v2-recovery/Cargo.toml", "sources/rust-manifest.toml"),
    cargoLock: copyNamed("rust-kat-lockfile", "designs/pf10/crates/shieldkit-v2-recovery/Cargo.lock", "sources/rust-lock.lock"),
    rustToolchain: copyNamed("rust-toolchain", "designs/pf10/rust-toolchain.toml", "sources/rust-toolchain.toml"),
...
function publicEvidence({ git, sources, implementations, histories, runtime, depth4, inputManifest, rawOutput, resultTranscript, q04 }) {
  const evidence = {
    ...
    provenance: { generatedAt: timestamp(), commands: {
      campaign: { executable: "node", arguments: ["designs/pf10/scripts/v2-q04-campaign.mjs"], workingDirectory: "." },
      rustKatBuild: { executable: "cargo", arguments: ["+1.97.1", "build", "--locked", "--release", "--bin", "q04-poseidon-oracle"], workingDirectory: "designs/pf10/crates/shieldkit-v2-recovery" },
      rustKatRun: { executable: "target/release/q04-poseidon-oracle", arguments: [], workingDirectory: "designs/pf10/crates/shieldkit-v2-recovery" },
      depth4CheckerBuild: { executable: "cargo", arguments: ["+1.97.1", "build", "--locked", "--release", "--bin", "shieldkit-v2-q04-certificate"], workingDirectory: "designs/pf10/crates/shieldkit-v2-q04-certificate" },
      depth4CheckerRun: { executable: "bin/shieldkit-v2-q04-certificate", arguments: ["raw/depth4-symbolic-certificate.json", "snapshot/designs/pf10/packages/pool/v2/persistent-indexed-nullifier.mjs", "snapshot/designs/pf10/crates/shieldkit-v2-q04-certificate/src/main.rs"], workingDirectory: "." },
...
export async function runQ04Campaign({ outputParent, dependencies = undefined } = {}) {
  ...
  try {
    ...
    const resultTranscript = bundleReference(bundle, writeJson(join(bundle, "raw/result-transcript.json"), {
      ...
      verifier: { executable: "node", arguments: ["designs/pf10/scripts/v2-q04-evidence-verify.mjs", "evidence.json"] },
    }));
