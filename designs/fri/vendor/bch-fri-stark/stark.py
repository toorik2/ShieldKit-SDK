...
try:
    from native_ct_air_config import MERKLE_HASH_BYTES
except Exception:
...
# ---- hashing ----
# Full SHA256 for Fiat-Shamir sponge (always 32B state — must match on-chain OP_SHA256).
# Truncated SHA256 only for Merkle tree nodes (N3 MERKLE_HASH_BYTES lever).
# Named sha256f / Hf (not H) because H is the trace domain list below.
def sha256f(b):
    return hashlib.sha256(b).digest()

def Hf(b):
    """Merkle node hash: full SHA256 when MERKLE_HASH_BYTES>=32, else first-n truncation."""
    h = hashlib.sha256(b).digest()
    return h if MERKLE_HASH_BYTES >= 32 else h[:MERKLE_HASH_BYTES]
...
# ---- Fiat-Shamir transcript (full SHA256 sponge — never truncate state) ----
class FS:
    def __init__(self): self.s=b"STARK-v0"
    def absorb(self,b): self.s=sha256f(self.s+b)
    def absorb_int(self,v): self.absorb(enc(v%P))
    def challenge(self): self.s=sha256f(self.s+b"chal"); return int.from_bytes(self.s[:8],'little')%P
    def challenge_idx(self,N): self.s=sha256f(self.s+b"idx"); return int.from_bytes(self.s[:8],'little')%N
...
def grind(transcript_state, bits):
    ...
    i=0
    while True:
        h=sha256f(transcript_state+i.to_bytes(8,'little'))  # full SHA256 (FS sponge)
...
def verify(proof):
    ...
    if int.from_bytes(sha256f(fs.s+nonce)[:8],'little') >= (1<<(64-grind_bits)): return False,"grinding fail"
    fs.absorb(nonce)
    idxs=[fs.challenge_idx(N) for _ in range(queries)]
