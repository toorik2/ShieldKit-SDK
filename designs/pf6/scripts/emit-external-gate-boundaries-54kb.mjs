...
const GATE_MODULE = path.resolve(repositoryRoot, '../designs/pf10/scripts/external-release-gate.mjs');
...
export function emitExternalGateBoundaries({ output = path.join(repositoryRoot, 'evidence/08-release/external-gate-boundaries.json') } = {}) {
  ...
  const record = {
    ...
    gateModule: {
      path: 'designs/pf10/scripts/external-release-gate.mjs (read-only reference)',
      sha256: `sha256:${sha256File(GATE_MODULE)}`,
    },
    command: GATES.map((gate) => `node designs/pf10/scripts/external-release-gate.mjs --gate ${gate}`),
    boundaries,
  };
