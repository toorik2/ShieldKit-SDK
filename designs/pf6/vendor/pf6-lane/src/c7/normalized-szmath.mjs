// Lossless fixed-G2-addition projective quotient for the C7 static-table path.
//
// Each gamma/delta addition line is divided by its nonzero c2 Fp2 coefficient.
// The resulting product differs from the ordinary Miller product only by an
// Fp2 factor, which final exponentiation eliminates.  Unlike a table-only
// rewrite, this module regenerates every dependent S-Z value from that exact
// quotient trajectory: residue witness, chain, Fiat-Shamir challenges, big-Q,
// and per-step evaluated factors.
import * as base from '../../../../build/chunked/pairing/_szmath.mjs';
import { Fp12, Fp2, f12limbs } from '../../../../build/chunked/pairing/_millermath.mjs';
import { residueWitnessForTerminal } from './residue-selector-reps.mjs';

export * from '../../../../build/chunked/pairing/_szmath.mjs';

// All fixed-G2 additions, including the two final `pp` lines per fixed pair,
// are quotient-normalized. Every omitted c2 is a nonzero Fp2 scalar, so the
// aggregate differs only by a final-exponent-trivial Fp2 factor.
const normalizedFixedAdd = (op) => (op.t === 'al' || op.t === 'pp') && (op.j === 2 || op.j === 3);
const normalizedTerminalFixedDouble = (op, opIndex, original) => (
  op.t === 'dl'
  && (op.j === 2 || op.j === 3)
  // The closing square is followed by pair0/fixed-g2 doubles, then pp. The
  // two fixed doubles are literal-only terminal factors, so this saves their
  // c2 limbs without changing the static executor's compact table codec.
...
const lineFactors = (t, op, opIndex, c, cInv, rawStates) => {
  ...
  return records.map((coeffs) => base.lineFp12(
    normalizedFixedAdd(op) || normalizedTerminalFixedDouble(op, opIndex, t)
...
const run = (original, c, cInv) => {
  ...
  for (let index = 0; index < original.fused.ops.length; index++) {
...
const rebuildSteps = (original, fused, c, cInv) => {
  const steps = [];
  let current = null;
  for (let index = 0; index < fused.ops.length; index++) {
...
export function trajectory() {
  ...
  if (process.env.SZ_ALLAFF !== '1') throw new Error('normalized fixed additions require the all-affine C7 trajectory');
  const original = base.trajectory();
  const raw = run(original, Fp12.ONE, Fp12.ONE);
  const { c, cInv, w, terminalW } = residueWitnessForTerminal(raw.boundary);
  ...
  // `residue` is intentionally not a construction invariant: malformed or
  // off-subgroup proofs reach the terminal so its residue relation rejects
  // them.  The identity and canonicality are required for every trajectory.
  if (!id.holds || !canonical.all) {
    throw new Error(`normalized fixed-add S-Z trajectory invariant failed: ${JSON.stringify({ identity: id.holds, canonical: canonical.all })}`);
  }
  cached = {
    ...original,
    c,
    ...
    prodFactorZ,
    id,
    fAB: original.fused.fAB,
    boundary: fused.boundary,
    normalizedFixedAdds: true,
  };
  return cached;
}
...
export function stepFactorSpec(t, index) {
  return base.stepFactorSpec(t, index).map((factor) => (
    (factor.kind === 'fixline' && (factor.op === 'al' || (factor.op === 'dl' && index === t.steps.length - 1)))
      || factor.kind === 'fixpp'
      ? { ...factor, coeffs: normalizeCoeffs(factor.coeffs) }
      : factor
  ));
}
