...
// the densDrop windows (verified for deposit; apply per-tx after verification)
for (const name of ['deposit', 'transfer', 'withdrawal']) {
  const { inputs, outputs } = load(name);
  const r = rng(555 + name.length);
  let accepts = 0, total = 0;
  for (let idx = 0; idx < 6; idx++) {
    const u = inputs[idx].unlockingBytecode;
    // 80 random flips OUTSIDE the known densDrop windows (deposit windows; for transfer/withdrawal use the same scan to find windows first — here we just use the deposit's windows as the hypothesis)
    for (let s = 0; s < 80; s++) {
      const k = r() % u.length;
      const inWindow = (idx === 4 && k >= 2927 && k <= 3045) || (idx === 5 && k >= 1926 && k <= 3833);
  ...
  console.log(`${name}: scoped corpus (authenticated bytes only): ${accepts}/${total} accepted (expect 0)`);
}
