...
//   cse-pass/cse_pass.mjs   (sound by Stackcert.InvokeCSE.invoke_cse)
//   fold-pass/fold_pass.mjs (sound by Stackcert.FoldTable + Peephole)
...
for (let round = 1; ; round++) {
  ...
  const c = runTool('cse-pass/cse_pass.mjs', outPath, tmpC);
  if (c < cur) { copyFileSync(tmpC, outPath); cur = c; }
  const f = runTool('fold-pass/fold_pass.mjs', outPath, tmpF);
