...
let best=Infinity,bestO=null,bestC=null;
const order=[];
function rec(done){
  if(done===(1<<ids.length)-1){ const nodeOrder=order.map(i=>ids[i]);
    for(const cfg of CONFIGS){ try{const ops=peephole(scheduleBlock(b,arity,{...cfg,nodeOrder}).ops); const s=serializeOps(ops).length; if(s<best){best=s;bestO=nodeOrder.slice();bestC=cfg;}}catch(e){} } return; }
  for(let i=0;i<ids.length;i++){ if(done&(1<<i))continue; if((predMask[i]&done)===predMask[i]){ order.push(i); rec(done|(1<<i)); order.pop(); } }
}
rec(0);
console.log(`blk#14 all16cfg exhaustive BEST=${best} save=${966-best} cfg=${JSON.stringify(bestC)}`);
console.log('order=',JSON.stringify(bestO));
