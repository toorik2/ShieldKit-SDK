/-
  ...
  / `MovementFloor.runMoves` (built on the project's real `Recompiler.removeNth`, the deletion
  `Op.ROLL` uses, over `Recompiler.Floor.Stack = List Nat`) — so the loose disjoint-pairs floor and
  ...
  lives in the nested `Recompiler.NlcsFloor` namespace (parallel to `Recompiler.MovementFloor`).
...
-- built on `Recompiler.removeNth`. `moveToTop_faithful_ROLL` + the SWAP/ROT/ROLL examples grounding
