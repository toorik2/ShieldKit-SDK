...
// correct source mapping: verifier i -> genesis vout i+1; packet -> vout 7; state -> vout 0; fee -> its own
...
for (let i = 0; i < inputs.length; i++) {
  const out = evaluatePair(vm, inputs[i].lockingBytecode, inputs[i].unlockingBytecode, undefined, { index: i, inputs, outputValueSatoshis: 1000n });
  console.log(`input[${i}] accepts=${out.accepted === true} ${out.error ?? ''} len=${inputs[i].unlockingBytecode.length}`);
}
