...
// the item at depth 73 (the PICK 73's target = the 73rd from the top = index length-1-73)
const pickIdx = stack.length - 1 - 73;
console.log('pick target index:', pickIdx);
console.log('value:', stack[pickIdx] !== undefined ? la.binToHex(stack[pickIdx]) : 'MISSING', '| len:', stack[pickIdx]?.length);
// the top few items
for (let k = stack.length - 5; k < stack.length; k++) {
  console.log('top', k, la.binToHex(stack[k]).slice(0, 40), 'len', stack[k].length);
}
