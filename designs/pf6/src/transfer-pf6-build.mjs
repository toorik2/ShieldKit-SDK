// ShieldKit-54KB — pf6 TRANSFER: prove -> adapter -> candidate -> (lane build separate) -> assemble.
...
const { hexToBin, binToHex, decodeTransaction, encodeTransaction, generateSigningSerializationBch, hash256, secp256k1, encodeDataPush } = la;
...
const wallet = JSON.parse(readFileSync(path.join(CUSTODY, 'wallet-public.json'), 'utf8'));
const walletPriv = JSON.parse(readFileSync(path.join(CUSTODY, 'wallet-private.json'), 'utf8'));
const priv = Uint8Array.from(Buffer.from(walletPriv.privateKeyHex, 'hex'));
const pubHex = walletPriv.publicKeyHex;
...
// ---- 1) prove ----
const outDir = path.join(FOLDER, 'evidence/03-implementation/transfer-prove');
...
const proved = prove.proveGroth16({
  ...
  circuitInputPath: '/tmp/pf6-transfer-input.json',
  outDir,
...
console.log('transfer proof ok | public:', publicSignals.map(String).join(',').slice(0, 30));

// ---- 2) adapter ----
const sha = (p) => createHash('sha256').update(readFileSync(p)).digest('hex');
const adapter = {
  schema: 'shieldkit-v2-direct-groth16-adapter-v1',
  qualification: 'shieldkit-54kb live pf6 transfer; VK d38f3cfc',
  source: {
...
writeFileSync('/tmp/pf6-transfer-adapter.json', JSON.stringify(adapter, null, 1));

// ---- 3) candidate ----
const candidate = {
  schema: 'verifier.cash/candidate/v1',
  id: 'bn254-onetx-pf6-a3-shieldkit-transfer-r1',
  lane: 'bn254-onetx', status: 'live-transfer',
  ...
  identity: { humanName: 'pf6 live transfer', construction: 'pf6-a3-shieldkit-transfer', topology: 6, stateModel: 'p2shchain', revision: 'r1', slug: 'pf6-transfer' },
  ...
  build: {
    ...
    shieldAdapter: { path: '/tmp/pf6-transfer-adapter.json', sha256: sha('/tmp/pf6-transfer-adapter.json') },
    shieldActionPacket: { path: '/tmp/pf6-transfer-packet.bin', sha256: sha('/tmp/pf6-transfer-packet.bin') },
  ...
  judge: { profile: 'shieldkit-pf6-live-transfer', sourceValueSatoshis: 1000, sequenceNumber: 2, expected: { scriptBytes: null, txOverheadBytes: null, score: null, inputCount: 9, transactionCount: 1 } },
  provenance: { sourceCommit: 'current-worktree', scope: 'live pf6 transfer on chipnet; pool ' + instanceId.slice(0, 16) },
};
mkdirSync('/tmp/pf6-transfer-run', { recursive: true });
writeFileSync('/tmp/pf6-transfer-run/candidate.json', JSON.stringify(candidate, null, 1));
console.log('transfer candidate written — invoke the lane build next');
