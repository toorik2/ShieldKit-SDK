// PF6 design lifecycle used by the cross-design pipeline benchmark.
...
// The benchmark may override the design root for isolated fixtures.
const ROOT = process.env.SHIELDKIT_DESIGN_ROOT
  ? path.resolve(process.env.SHIELDKIT_DESIGN_ROOT)
  : path.resolve(HERE, '..');
const SDK_ROOT = path.resolve(ROOT, '../..');
const G = path.join(SDK_ROOT, 'designs/pf10');
...
const rpc = async () => {
  if (_rpc) return _rpc;
  const { createLayer1BchnChipnetRpc } = await import('file://' + path.join(G, 'packages/kit/chipnet-rpc.mjs'));
