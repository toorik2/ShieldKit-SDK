...
const outDir = path.join(FOLDER, 'evidence/03-implementation/cont-deposit-prove');
...
const proved = prove.proveGroth16({
  ...
  circuitInputPath: '/tmp/pf6-cont-input.json', outDir,
...
writeFileSync('/tmp/pf6-cont-adapter.json', JSON.stringify(adapter, null, 1));
const candidate = {
  schema: 'verifier.cash/candidate/v1',
  id: 'bn254-onetx-pf6-a3-shieldkit-cont-deposit-r1',
  lane: 'bn254-onetx', status: 'live-cont-deposit',
  ...
  identity: { humanName: 'pf6 continuation deposit (VPS pool)', construction: 'pf6-a3-shieldkit-cont-deposit', topology: 6, stateModel: 'p2shchain', revision: 'r1', slug: 'pf6-cont-deposit' },
  ...
  build: {
    adapter: 'lane-module', module: 'lanes/bn254-onetx/src/build-adapter.mjs', entrypoint: 'lanes/bn254-onetx/src/c7/build.ts',
    ...
    shieldAdapter: { path: '/tmp/pf6-cont-adapter.json', sha256: sha('/tmp/pf6-cont-adapter.json') },
    shieldActionPacket: { path: '/tmp/pf6-cont-packet.bin', sha256: sha('/tmp/pf6-cont-packet.bin') },
  ...
  judge: { profile: 'shieldkit-pf6-cont-deposit', sourceValueSatoshis: 1000, sequenceNumber: 4, expected: { scriptBytes: null, txOverheadBytes: null, score: null, inputCount: 9, transactionCount: 1 } },
  provenance: { sourceCommit: 'current-worktree', scope: 'continuation deposit on the mined VPS pool; instance ' + meta.instanceId.slice(0, 16) },
};
writeFileSync('/tmp/pf6-cont-run/candidate.json', JSON.stringify(candidate, null, 1));
console.log('cont candidate written');
