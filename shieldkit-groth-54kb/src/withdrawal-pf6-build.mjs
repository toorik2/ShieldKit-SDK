// ShieldKit-54KB — pf6 WITHDRAWAL: prove -> adapter -> candidate.
...
const outDir = path.join(FOLDER, 'evidence/03-implementation/withdrawal-prove');
...
const proved = prove.proveGroth16({
  ...
  circuitInputPath: '/tmp/pf6-withdrawal-input.json',
  outDir,
...
console.log('withdrawal proof ok');
const sha = (p) => createHash('sha256').update(readFileSync(p)).digest('hex');
const adapter = {
  schema: 'shieldkit-v2-direct-groth16-adapter-v1',
  qualification: 'shieldkit-54kb live pf6 withdrawal; VK d38f3cfc',
  source: {
...
writeFileSync('/tmp/pf6-withdrawal-adapter.json', JSON.stringify(adapter, null, 1));
const candidate = {
  schema: 'verifier.cash/candidate/v1',
  id: 'bn254-onetx-pf6-a3-shieldkit-withdrawal-r1',
  lane: 'bn254-onetx', status: 'live-withdrawal',
  ...
  identity: { humanName: 'pf6 live withdrawal', construction: 'pf6-a3-shieldkit-withdrawal', topology: 6, stateModel: 'p2shchain', revision: 'r1', slug: 'pf6-withdrawal' },
  ...
  build: {
    ...
    shieldAdapter: { path: '/tmp/pf6-withdrawal-adapter.json', sha256: sha('/tmp/pf6-withdrawal-adapter.json') },
    shieldActionPacket: { path: '/tmp/pf6-withdrawal-packet.bin', sha256: sha('/tmp/pf6-withdrawal-packet.bin') },
  ...
  judge: { profile: 'shieldkit-pf6-live-withdrawal', sourceValueSatoshis: 1000, sequenceNumber: 3, expected: { scriptBytes: null, txOverheadBytes: null, score: null, inputCount: 9, transactionCount: 1 } },
  provenance: { sourceCommit: 'current-worktree', scope: 'live pf6 withdrawal on chipnet; pool ' + gen.instanceId.slice(0, 16) },
};
mkdirSync('/tmp/pf6-withdrawal-run', { recursive: true });
writeFileSync('/tmp/pf6-withdrawal-run/candidate.json', JSON.stringify(candidate, null, 1));
console.log('withdrawal candidate written');
