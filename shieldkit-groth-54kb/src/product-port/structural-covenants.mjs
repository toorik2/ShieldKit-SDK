...
} from './topology-product.mjs';
import { resolveDirectV2Pf6VerifierTopology as resolvePf6, DIRECT_V2_PF6_TOPOLOGY_ID, DIRECT_V2_PF6_VERIFIER_ROLES } from '../topology-pf6.mjs';
...
function verifierTopology({
  ...
  if (topologyId === DIRECT_V2_PF6_TOPOLOGY_ID) {
    return resolvePf6({ id: topologyId, verifierRoles });
  }
...
function exactP2sh32Lock(value, label) {
  // pf6: the packet carrier lock is OP_DROP OP_TRUE (75 51, design 01 v3) —
  // consumes the packet push, leaves exactly one stack item; the terminal
  // guard binds the packet content. (OP_1 alone was unspendable: final stack
  // depth 2 under the BCH VM rule.)
  if (value.length === 2 && value[0] === 0x75 && value[1] === 0x51) return value;
  const bytes = exactBytes(value, 35, label);
  if (bytes[0] !== 0xaa || bytes[1] !== 0x20 || bytes[34] !== op.EQUAL) {
    fail(`${label} must be canonical P2SH32`);
  }
  return bytes;
}
...
function buildDirectV2BindingProgram({
  ...
  emitNumber(out, topology.stateOutputIndex);
  emit(out, op.OUTPUTTOKENCOMMITMENT, op.EQUALVERIFY);
...
export function buildDirectV2StateHelper({
  ...
  // pf6 layout: the pool's covenant outputs are verifier@0..5, carrier@6,
  // state@7 — input[i].vout == i (the verifier's txTopologyGuard enforces it).
  ...
  // pf6 layout: outputs mirror the pool (verifier@0-5, carrier@6, state@7)
  for (let index = 0; index < topology.carrierCount; index += 1) {
    requirePinnedInput(out, index, locks[index], values[index]);
    requirePinnedOutput(out, index, locks[index], values[index]);
    requireTokenlessInput(out, index);
    requireTokenlessOutput(out, index);
  }
  requirePinnedInput(
  ...
  emit(out, op.NUMEQUALVERIFY, op.DROP);
  emitNumber(out, topology.stateOutputIndex);
  emit(out, op.OUTPUTTOKENCOMMITMENT, op.SIZE);
  emitNumber(out, 128);
  emit(out, op.NUMEQUALVERIFY, op.DROP);
  ...
  emit(out, op.ADD);
  emitNumber(out, topology.stateOutputIndex);
  emit(out, op.OUTPUTVALUE, op.NUMEQUALVERIFY);
...
export function buildDirectV2StateTrampolineLock({
  ...
  emitNumber(out, topology.stateInputIndex);
  emit(out, op.NUMEQUALVERIFY, op.ACTIVEBYTECODE);
  emitNumber(out, topology.stateOutputIndex);
  emit(out, op.OUTPUTBYTECODE);
  emit(out, op.EQUALVERIFY);
  emitNumber(out, topology.bindingInputIndex);
  emit(out, op.UTXOBYTECODE, op.SHA256);
