/-
  ...
  STATUS: fully closed — `schedule_refines` (Compose.lean) is PROVEN, `sorry`-free
  (`[propext, Classical.choice, Quot.sound]`, no `sorryAx`). The relation, the value semantics,
  the entry base case (`inv_entry`), all four step-lemmas (`LeanBCH.Opt.StepLemmas`), the denotation
  link (node `sem` = DAG output), the operand-layout bookkeeping, and the `StepOk.trans` fold over
  the schedule are all machine-checked. (Earlier drafts of this header described a residual `sorry`
  frontier; those holes are discharged — the frontier is closed.) The one remaining GAP is external
  to this proof: `schedule_refines` is over the abstract `Opt.run`; grounding it in the real
  `stepInstr` is the (open) Adapter bridge — see `LeanBCH.Opt.Adapter`.
-/
...
/-! ### ★ END-TO-END REFINEMENT (the scheduler's correctness theorem).
    ...
    STATUS — the STEP-LEMMAS are now MACHINE-CHECKED (0 sorry) in `LeanBCH.Opt.StepLemmas`,
    ...
    ★ THE END-TO-END THEOREM `schedule_refines` IS NOW ASSEMBLED IN `LeanBCH.Opt.Compose`
