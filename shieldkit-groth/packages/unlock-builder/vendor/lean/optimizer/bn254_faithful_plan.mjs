...
const DEPLOYED_COSTVEC_PATH = join(HERE, 'bn254_costvec_deployed.json');
const REPORT_PATH = join(HERE, 'bn254_faithful_plan_report.json');
...
writeFileSync(DEPLOYED_COSTVEC_PATH, JSON.stringify({
  meta: { generator: 'gen_miller_residue.mjs', numOps: N, policy: 'consensus-64 deployed (compileFileBytecode rescheduleStacks ON, createVirtualMachineBch2026(false))',
    opBudget: OP_BUDGET, compiles, runStarts, WCAP, extractedAt: new Date().toISOString(),
    note: 'per-op-CLASS marginals fit on the DEPLOYED compile (fixes the RAW-compile ~40% under-count). marginMax >= marginMean; feasibility uses intercept+Σ marginMax vs opBudget-SAFETY_MARGIN.' },
