...
import { emitInvokeOps } from './program.mjs';   // dialect-backed invoke emit (the one format seam here)
...
export function scheduleBlock(block, arity, opts = {}) {
  ...
  const emitOne = (node) => {
    ...
    else { for (const o of emitInvokeOps(node.invId)) emit(o); }   // dialect owns id-push + INVOKE
  ...
  if (opts.nodeOrder) {
    // AGGRESSIVE search hook: emit nodes in a caller-supplied explicit order. The order
    // MUST be a valid topological order of `nodes` (deps before dependents); the search
    // driver only ever generates valid linear extensions, and any invalid order would
    // throw in materialize (home not found), so it can never silently miscompile.
    const seen = new Set();
    for (const nid of opts.nodeOrder) {
      const node = byId.get(nid);
      if (!node) throw new Error('scheduler: unknown node id in nodeOrder ' + nid);
      for (const p of depNodes(node)) if (!seen.has(p.id)) throw new Error('scheduler: nodeOrder not topological');
      emitOne(node); seen.add(nid);
    }
    if (seen.size !== nodes.length) throw new Error('scheduler: nodeOrder incomplete');
  } else if (readyOrder) {
  ...
  if (opts._probe) opts._probe({
    S: S.map((x) => ({ tok: x.tok, t: !!x.t })),
    exit: exit.map((r) => ({ tok: tokenOf(r), k: r.k, enc: r.enc })),
    opsBefore: ops.length,
  });
  ...
  // (b-MOVE) MOVE-BASED arrange (opts.arrange === 'move'). For permutation-heavy blocks
  // (few dups, mostly last-use) the copy+delete arrange grows the stack to ~2x and makes
  // every PICK a deep 2-byte access, then pays again to delete the stale tail. Instead:
  //   - drop dead homes (ROLL+DROP, shallowest first),
  //   - build exit[L..] by MOVING (ROLL) each token on its LAST exit occurrence and COPYING
  //     (PICK) earlier occurrences; consts are pushed (or copied from a live home).
  // Every value re-emits the identical arithmetic; only the placement ops differ, so this is
  // sound by construction and guarded by the per-subroutine differential + E2E. A final
  // self-check throws if the resulting main layout != exit (never silently miscompiles).
  if (opts.arrange === 'move') {
    // future-use counts within exit (by token)
    const need = new Map();
    for (const r of exit) { const t = tokenOf(r); if (t !== null) need.set(t, (need.get(t) || 0) + 1); }
    const homeUnplaced = (tok, placed) => depthOfHome(tok, S.length - placed);
    // (1) drop dead homes above the kept prefix (shallowest first)
    for (;;) {
      let best = -1, bestDepth = Infinity;
      for (let idx = L; idx < S.length; idx++) {
        const e = S[idx]; if (e.t) continue;
        if (e.tok !== null && !need.has(e.tok)) { const dep = S.length - 1 - idx; if (dep < bestDepth) { bestDepth = dep; best = idx; } }
      }
      if (best < 0) break;
      const d = S.length - 1 - best;
      if (d === 0) { emit({ op: OP.DROP }); S.splice(best, 1); }
      else if (d === 1) { emit({ op: 0x77 }); S.splice(best, 1); } // NIP
      else { for (const b of pushIntOps(d)) emit(b); emit({ op: OP.ROLL }); emit({ op: OP.DROP }); S.splice(best, 1); }
    }
    // (2) build exit[L..], moving on last use / copying otherwise
    let placed = 0;
    const seenAfter = new Map(); // remaining occurrences to the RIGHT (incl current) as we scan
    for (const r of exit) { const t = tokenOf(r); if (t !== null) seenAfter.set(t, (seenAfter.get(t) || 0) + 1); }
    for (let e = 0; e < L; e++) { const t = tokenOf(exit[e]); if (t !== null) seenAfter.set(t, seenAfter.get(t) - 1); }
    for (let e = L; e < exit.length; e++) {
      const ref = exit[e];
      const tok = tokenOf(ref);
      if (ref.k === 'const' && tok === null) { for (const b of encConst(ref)) emit(b); S.push({ tok: null }); placed++; continue; }
      const rem = (seenAfter.get(tok) || 0) - 1; // occurrences strictly after this one
      seenAfter.set(tok, rem);
      const isLast = rem <= 0;
      if (isLast) {
        // MOVE the original home from the unplaced region
        const d = homeUnplaced(tok, placed);
        if (d < 0) {
          if (ref.k === 'const') { for (const b of encConst(ref)) emit(b); S.push({ tok }); placed++; continue; }
          throw new Error('scheduler(move): home not found for ' + tok);
        }
        const idx = S.length - 1 - d;
        S.splice(idx, 1);
        ...
        // COPY from the shallowest live instance (placed copies are fine to copy from)
        const d = depthOfHome(tok);
        if (d < 0) {
          if (ref.k === 'const') { for (const b of encConst(ref)) emit(b); S.push({ tok }); placed++; continue; }
          throw new Error('scheduler(move-copy): home not found for ' + tok);
        }
        if (d === 0) emit({ op: OP.DUP });
        else if (d === 1) emit({ op: OP.OVER });
        else { for (const b of pushIntOps(d)) emit(b); emit({ op: OP.PICK }); }
    ...
    // (3) self-check: main must now equal exit exactly (bottom->top)
    if (S.length !== exit.length) throw new Error('scheduler(move): length ' + S.length + ' != exit ' + exit.length);
    for (let i = 0; i < exit.length; i++) {
      const want = tokenOf(exit[i]);
      if (want === null) { if (!(S[i].tok === null)) throw new Error('scheduler(move): const slot ' + i); }
      else if (S[i].tok !== want) throw new Error('scheduler(move): slot ' + i + ' ' + S[i].tok + ' != ' + want);
    }
    return { ops };
  }
