...
def main : IO Unit := do
  ...
  let txHex ← IO.FS.readFile "/tmp/xcheck_tx.hex"
  let soHex ← IO.FS.readFile "/tmp/xcheck_srcouts.hex"
  let txB := hexToBytes txHex.trim.toList
  let soB := hexToBytes soHex.trim.toList
  match decodeProgram txB soB 0 with
  ...
  | some p =>
      ...
      IO.println s!"ORACLE={oracleName} leanVerifyInput={acc} txValid={tv} verifyTokens={tk} leanFullOpCost={cost}"
