...
} from '../recover/recovery.mjs';
import { recoverAuthenticatedChainHistory, serializeChainHistoryActions } from '../recover/chain-history.mjs';
...
/** Networks accepted by kit coordinates (broadcast gates still apply separately). */
export const SUPPORTED_NETWORKS = Object.freeze(['chipnet', 'mainnet']);
...
export function createProfileCoordinates(value) {
  ...
  if (!SUPPORTED_NETWORKS.includes(value.network)) {
    fail(
      'UNSUPPORTED_NETWORK',
