...
test('public action index is PF10-only (no g2 / legacy assemble surface)', async () => {
  const action = await import('../action/index.mjs');
  const keys = Object.keys(action);
  const bad = keys.filter((k) => /g2|G2|assembleCompleteSettlement|planCompleteSettlement/i.test(k));
  assert.deepEqual(bad, [], `forbidden public names must be absent, got: ${bad.join(',')}`);
  assert.equal(typeof action.encodeActionPacket, 'function');
  assert.equal(typeof action.decodeActionPacket, 'function');
  assert.equal(typeof action.planCompleteSettlement, 'undefined');
  assert.equal(typeof action.assembleCompleteSettlement, 'undefined');
});

test('PF10 product modules exist (legacy seven-carrier unlock/build moved out)', () => {
  ...
  assert.ok(existsSync(path.join(packagesRoot, 'prove/v2-direct-groth16-adapter.mjs')));
  assert.ok(existsSync(path.join(packagesRoot, 'unlock-builder/index.mjs')));
  assert.ok(existsSync(path.join(packagesRoot, 'unlock-builder/v2/pf10-action-witness.mjs')));
  assert.equal(existsSync(path.join(packagesRoot, 'prove/unlock.mjs')), false);
  assert.equal(existsSync(path.join(packagesRoot, 'unlock-builder/build.mjs')), false);
});
