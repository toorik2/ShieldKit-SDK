...
// tuple into the fixture field names consumed by the historical verifier.cash carrier field names. It does
...
function verificationKey(raw) {
  ...
  return Object.freeze({
    ...
    // may not. A downstream verifier pin may impose a stricter finite-IC import gate downstream.
...
/**
 * Convert an immutable snarkjs tuple to the historical carrier field names used by
...
export async function adaptSnarkjsGroth16(records) {
  ...
  return Object.freeze({
    ...
    qualification: 'local conversion only; not a verifier bundle, profile, setup, standardness, or deployment result; canonical infinity IC0 is valid source material but incompatible with the historical seven-carrier builder and rejected downstream without substitution; that builder rejects all adapter input because its gb3/SZ/FIXED_G2 trajectories are static',
    ...
    byteOrder: Object.freeze({
      scalars: 'canonical unsigned base-10 JSON strings; downstream pins convert to their VM-number representation',
