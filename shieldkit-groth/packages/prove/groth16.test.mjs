...
test('adapts a real snarkjs-verified two-public BN254 Groth16 fixture into historical carrier fields', async () => {
  ...
  assert.match(result.qualification, /infinity IC0 is valid source material but incompatible with the historical seven-carrier builder/);
  assert.match(result.qualification, /that builder rejects all adapter input/);
...
test('emits a V2 Direct development-proof adapter with no historical seven-carrier schema identity', async () => {
  const result = await adaptV2DirectGroth16(await records());
  assert.equal(result.schema, V2_DIRECT_GROTH16_ADAPTER_SCHEMA);
  assert.doesNotMatch(result.schema, /dens-fuel/i);
  assert.doesNotMatch(result.qualification, /dens-fuel/i);
...
test('exports only the explicit historical carrier fixture field names', async () => {
  const result = await adaptSnarkjsGroth16(await records());
...
test('CLI requires a caller-pinned manifest and emits the same historical carrier fixture', async () => {
  const directory = await mkdtemp(path.join(here, '.tmp-adapter-'));
  try {
