...
const DESIGN_ROOT = path.join(SDK_ROOT, 'designs/pf6');
const PROFILE_MODULE = path.join(DESIGN_ROOT, 'src/lifecycle-profile.mjs');
const PROFILE_RUNNER = path.join(SDK_ROOT, 'bench/pipeline/adapters/run-pf6-profile.mjs');
...
export function runPf6Action(opts) {
  ...
  if (!existsSync(DESIGN_ROOT)) {
    return buildFailClosedRun({
      ...
      reason: 'PF6 design root missing',
      cacheMode,
      environment: env,
    });
  }
