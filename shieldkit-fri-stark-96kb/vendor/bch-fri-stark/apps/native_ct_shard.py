...
_EXT_MUL_FID = b"\x30"
_EXT_SUB_FID = b"\x31"
_EXT_SCALAR_FID = b"\x32"


def _field_prelude():
    """p resident on the alt stack + the MULMOD/ADDMOD/SUBMOD function table.

    BYTE-LEVER: also DEFINE GF(p^2) add/mul/sub/scalar once (0x2f–0x32) so every inlined
    ext_*_stack_prog call becomes a 3-byte INVOKE — large win on chain/state/FRI-adjacent code."""
    ...
    # Install GF(p^2) helpers after base field table (their bodies INVOKE MULMOD/ADDMOD/SUBMOD).
    prog += [PUSH(_EXT_ADD_FID), DEFINE(_ext_add_stack_body())]
    prog += [PUSH(_EXT_SUB_FID), DEFINE(_ext_sub_stack_body())]
    prog += [PUSH(_EXT_MUL_FID), DEFINE(_ext_mul_stack_body())]
    prog += [PUSH(_EXT_SCALAR_FID), DEFINE(_ext_scalar_stack_body())]
    return prog
...
def _ext_add_stack_body():
    """Raw GF(p^2) add body (for OP_DEFINE); see ext_add_stack_prog."""
...
def _ext_mul_stack_body():
    def body(pick, num, mm, am, sm):
        pick("a0"); pick("b0"); mm(); pick("a1"); pick("b1"); mm(); num(EXT_W); mm(); am()
        pick("a0"); pick("a1"); am(); pick("b0"); pick("b1"); am(); mm()
        pick("a0"); pick("b0"); mm(); sm(); pick("a1"); pick("b1"); mm(); sm()
    return _ext_stack(["a0", "a1", "b0", "b1"], body)


def ext_add_stack_prog():
    """[a0,a1,b0,b1] -> [a0+b0, a1+b1] (drops the 4 operands). INVOKE 0x2f (installed by _field_prelude)."""
    return [PUSH(_EXT_ADD_FID), OP("INVOKE")]


def ext_sub_stack_prog():
    """[a0,a1,b0,b1] -> [a0-b0, a1-b1]. INVOKE 0x31."""
    return [PUSH(_EXT_SUB_FID), OP("INVOKE")]


def ext_scalar_stack_prog():
    """[s, a0, a1] -> [s*a0, s*a1] (base scalar s times GF(p^2) a). INVOKE 0x32."""
    return [PUSH(_EXT_SCALAR_FID), OP("INVOKE")]


def ext_mul_stack_prog():
    """[a0,a1,b0,b1] -> Karatsuba [c0,c1]. INVOKE 0x30."""
    return [PUSH(_EXT_MUL_FID), OP("INVOKE")]
...
def deep_sum_part_counter_dual_prog(N, nz, nzg):
    ...
    prog += ([OP("TOALT"), OP("TOALT"), OP("TOALT"), OP("TOALT")] + drop_n_prog(6 * N)
...
def _deep_dedup_mac_loop_hoisted4(nf, off, extra, B, ba, bo):
    """PCO-DEEP-04 dens op cut: pure-trace dedup MAC loop with hoisted four=4*t.

    Same term schedule as `_deep_strided_mac_loop` with opspec
    [(ba,4,off),(ba+1,4,off),(0,1,0),const0,(bo,4,off),(bo+1,4,off)] — i.e. value-single-source
    z (off=0) / zg (off=2) terms — but keeps a running `four=4*t` register so each mult=4
    address is `base+four` instead of re-computing `NUM(4) MUL` from t on every operand.
    Algebraically identical MAC terms (same abs cells, same lazy 0x29 body). Alt-stack: parks
    acc above P only transiently (net-0) so NORM_TAIL still sees P.

    Frame: [blocks, extra..., t, four, acc0, acc1]. Leaves [blocks, extra..., acc0_red, acc1_red]."""
    p = [NUM(0), NUM(0), NUM(0), NUM(0)]  # t, four, acc0, acc1
    body = []
    # operands: alpha0, alpha1, value0, value1=0, ood0, ood1
    specs = [
        ("4", ba + off),
        ("4", ba + off + 1),
        ("1", 0),
    ...
    for i, (kind, base) in enumerate(specs):
        if kind == "0":
            body += [NUM(0)]
            continue
        # After i operand pushes: four@2+i, t@3+i; depth(ap)=B+extra+3+i-ap
        if kind == "4":
            C = B + extra + 3 - base  # depth = C+i - four
            body += [NUM(2 + i), OP("PICK"), NUM(C + i), OP("SWAP"), OP("SUB"), OP("PICK")]
        else:
            C = B + extra + 3  # depth = C+i - t  (value abs = t)
            body += [NUM(3 + i), OP("PICK"), NUM(C + i), OP("SWAP"), OP("SUB"), OP("PICK")]
    body += [PUSH(_DEEP_MAC_LAZY_FID), OP("INVOKE")]
    # park acc above P, t+=1, four+=4, restore acc (alt net-0)
    body += [OP("TOALT"), OP("TOALT")]
    body += [OP("SWAP"), NUM(1), OP("ADD"), OP("SWAP")]  # t += 1
    body += [NUM(4), OP("ADD")]                            # four += 4
    body += [OP("FROMALT"), OP("FROMALT")]
    body += [NUM(3), OP("PICK"), NUM(nf), OP("NUMEQUAL")]  # exit t == nf
    p += [OP("BEGIN")] + body + [OP("UNTIL")]
    p += [OP("TOALT"), OP("TOALT"), OP("DROP"), OP("DROP"), OP("FROMALT"), OP("FROMALT")]  # drop t,four
    p += [OP("SWAP")] + NORM_TAIL + [OP("SWAP")] + NORM_TAIL
    return p
...
def deep_sum_part_dedup_prog(nf):
    ...
    PCO-DEEP-04: uses `_deep_dedup_mac_loop_hoisted4` (running four=4*t) instead of per-operand MUL — same cells/MAC,
    lower op-cost on the dens-bound deepquery path. Formal: FriStark.Optimizer.HoistedFour.hoisted4_addr_eq_mult."""
    ba, bo = nf, 5 * nf
    B = 9 * nf
    prog = [PUSH(_DEEP_MAC_LAZY_FID), DEFINE(_deep_mac_lazy_body())]
    prog += _deep_dedup_mac_loop_hoisted4(nf, 0, 0, B, ba, bo)   # z-terms (2c) -> Sum_z
    prog += _deep_dedup_mac_loop_hoisted4(nf, 2, 2, B, ba, bo)   # zg-terms (2c+1) -> Sum_zg
    prog += ([OP("TOALT"), OP("TOALT"), OP("TOALT"), OP("TOALT")] + drop_n_prog(B)
...
def _ext_vstack(op_names, body):
    ...
    n_res = above[0] // 2
    prog += [OP("TOALT")] * (2 * n_res) + drop_n_prog(2 * len(op_names)) + [OP("FROMALT")] * (2 * n_res)
    return prog
...
_CAP_MAD_FID = b"\x2c"  # capacity base-scalar MAD: [acc0,acc1,v0,v1,c] -> acc + c*v
_YFULL_FID = b"\x2d"    # y_full lane: [s0,s1,rc0,rc1,u60,u61] -> [y0,y1] = u6*(s+rc)


def _cap_mad_body():
    """BYTE-LEVER: [acc0, acc1, v0, v1, c] -> [acc0+c*v0, acc1+c*v1] (base c × GF(p^2) v, add into acc).

    Needs field prelude (P on alt + MULMOD). Validated == ext_scalar + ext_add. Body ~52B; INVOKE once per
    (row, lane) replaces an inlined scalar+add so chain_ext redeem collapses ~5k → ~1k."""
    prog = [NUM(0), OP("PICK"), NUM(3), OP("PICK"), NUM(3), OP("PICK")]  # push [c, v0, v1]
    prog += ext_scalar_stack_prog()                                       # -> [acc0,acc1,v0,v1,c, cv0,cv1]
    prog += [OP("TOALT"), OP("TOALT"), OP("DROP"), OP("DROP"), OP("DROP"),
             OP("FROMALT"), OP("FROMALT")]                                 # drop v,c; keep [acc, cv]
    prog += ext_add_stack_prog()
    return prog


def _yfull_body():
    """BYTE-LEVER: [s0,s1,rc0,rc1,u60,u61] -> [y0,y1] with y = u6*(s+rc) over GF(p^2)."""
    # s+rc then *u6
    prog = list(ext_add_stack_prog())   # need [s,rc] on top — stack is s,rc,u6 with u6 on top
    # reorder: currently bottom->top s0,s1,rc0,rc1,u60,u61
    # want add(s,rc) then mul u6
    # pick-based: compute s+rc into temp
    prog = []
    # push s, rc via picks then add; then pick u6; mul
    # stack: s0,s1,rc0,rc1,u60,u61 (top=u61)
    prog += [NUM(5), OP("PICK"), NUM(5), OP("PICK")]  # s0,s1
    prog += [NUM(5), OP("PICK"), NUM(5), OP("PICK")]  # rc0,rc1 (depths shifted)
    prog += ext_add_stack_prog()                       # [s,rc,u6, s+rc]
    prog += [NUM(3), OP("PICK"), NUM(3), OP("PICK")]  # u6
    prog += ext_mul_stack_prog()                       # [s,rc,u6, y]
    prog += [OP("TOALT"), OP("TOALT")] + drop_n_prog(6) + [OP("FROMALT"), OP("FROMALT")]
    return prog
...
def chain_ext_residuals_prog(minv_cap_rows):
    ...
    uses a base row (m.scalar via DEFINE 0x2c MAD), the chaining uses the ext chain_minv (m.mul). Drops the operand limbs."""
    ops = (["cs%d" % j for j in range(WIDTH)] + ["ns%d" % j for j in range(WIDTH)]
           + ["isbs", "isra"] + ["mc%d" % j for j in range(WIDTH)])
    # BYTE-LEVER: DEFINE capacity MAD once; each (row,j) is PICK cs_j + NUM(coeff) + INVOKE (~15B) vs inlined scalar+add.
    prog = [PUSH(_CAP_MAD_FID), DEFINE(_cap_mad_body())]
    above = [0]
    limbs = [nm + "#%d" % c for nm in ops for c in (0, 1)]
    top_index = {nm: i for i, nm in enumerate(reversed(limbs))}
    ...
    def mul():
        prog.extend(ext_mul_stack_prog()); above[0] -= 2

    def add():
        prog.extend(ext_add_stack_prog()); above[0] -= 2

    def sub():
        prog.extend(ext_sub_stack_prog()); above[0] -= 2

    for row in minv_cap_rows:                                # capacity: isbs * (row . cur_state)
        prog.extend([NUM(0), NUM(0)]); above[0] += 2         # acc = 0
        for j in range(WIDTH):
            copy("cs%d" % j)
            prog.append(NUM(row[j] % P)); above[0] += 1      # c
            prog.extend([PUSH(_CAP_MAD_FID), OP("INVOKE")]); above[0] -= 3  # acc += c*cs_j
        copy("isbs"); mul()
    copy("mc0"); copy("ns0"); mul()                          # chaining: sum_j minv_chain_row[j] * nxt.s_j
    for j in range(1, WIDTH):
        copy("mc%d" % j); copy("ns%d" % j); mul(); add()
    copy("cs0"); sub()                                       # chained - cur.s0
    copy("isra"); mul()                                      # isra * (chained - cur.s0)
    n_res = len(minv_cap_rows) + 1
    assert above[0] == 2 * n_res, "chain_ext left %d limbs (expected %d)" % (above[0], 2 * n_res)
    prog += [OP("TOALT")] * (2 * n_res) + drop_n_prog(2 * len(ops)) + [OP("FROMALT")] * (2 * n_res)
    return prog
...
def boundary_ext_residuals_prog(minv0_row, n_out, range_spec, root, nf, cm_out, mad_defined=False,
                                public_from_blob=False):
    ...
    BYTE-LEVER: cm-bind matvec uses DEFINE 0x2c MAD (same as chain_ext / M4) instead of inlined scalar+add.
    mad_defined=True: skip DEFINE 0x2c (already installed — LEVER-6 last-trans+final fuse).

    public_from_blob=True (product fixed-locks): read root/nf/cm_out from sound blob@0 layout
    [root(8)|nf(8)|cm_out(8*n_out)|…] via OP_0 OP_INPUTBYTECODE instead of baking statement NUM
    into the redeem (redeems become profile-param-only; statement public lives in blob unlock)."""
    ops = ["s%d" % k for k in range(WIDTH)] + ["vh%d" % i for i in range(4)]
    need_mad = any(has for _, has in range_spec)
    prog = ([PUSH(_CAP_MAD_FID), DEFINE(_cap_mad_body())] if (need_mad and not mad_defined) else [])
    above = [0]
    limbs = [nm + "#%d" % c for nm in ops for c in (0, 1)]
    top_index = {nm: i for i, nm in enumerate(reversed(limbs))}
    ...
    def const(val):
        prog.extend([NUM(val[0] % P), NUM(val[1] % P)]); above[0] += 2

    def const_blob_u64(field_off):
        """Read 8B LE u64 from blob content at field_off → GF(p^2) (v, 0) on stack.

        Absolute blob index: VC_BLOB_IDX / VC_ROLE_INDEX_BASE / VC_INPUT_BASE (default 0).
        Product tip state@0 uses base=1 so blob sits at input 1."""
        # scriptSig = PUSH(nonce8)+PUSH(blob)+PUSH(redeem); blob content starts at byte 12.
        skip = 12 + field_off
        import os as _os
        try:
            _bidx = int(str(
                _os.environ.get("VC_BLOB_IDX")
                or _os.environ.get("VC_ROLE_INDEX_BASE")
                or _os.environ.get("VC_INPUT_BASE")
                or "0"
            ).strip() or "0")
        except ValueError:
            _bidx = 0
        if _bidx <= 16:
            ...
            idx_tok = "<0x%s>" % encode_num(_bidx).hex()
        # Emit as CashAssembly fragment (OP_INPUTBYTECODE is not a cashvm token opcode).
        asm = (
            "%s OP_INPUTBYTECODE <0x%s> OP_SPLIT OP_NIP <0x%s> OP_SPLIT OP_DROP "
            "<0x00> OP_CAT OP_BIN2NUM OP_0"
            % (idx_tok, encode_num(skip).hex(), encode_num(8).hex())
        )
        prog.append(("ASM", asm))
        above[0] += 2
    ...
    def swap():
        prog.extend([NUM(3), OP("ROLL"), NUM(3), OP("ROLL")])

    if public_from_blob:
        copy("s0"); const_blob_u64(0); sub()                   # root eq (blob[0:8])
        copy("s0"); const_blob_u64(8); sub()                   # nf eq (blob[8:16])
        for i in range(n_out):
            copy("s0"); const_blob_u64(16 + 8 * i); sub()      # cm_out[i]
    else:
        copy("s0"); const((root % P, 0)); sub()            # root eq
        copy("s0"); const((nf % P, 0)); sub()              # nf eq
        for i in range(n_out):
            copy("s0"); const((cm_out[i] % P, 0)); sub()
    for hv_idx, has_cmbind in range_spec:
        copy("vh%d" % hv_idx); copy("s2"); sub()       # link
        if has_cmbind:
            prog.extend([NUM(0), NUM(0)]); above[0] += 2
            for k in range(WIDTH):
                copy("s%d" % k)
                prog.append(NUM(minv0_row[k] % P)); above[0] += 1
                prog.extend([PUSH(_CAP_MAD_FID), OP("INVOKE")]); above[0] -= 3
            copy("vh%d" % hv_idx); swap(); sub()       # vh - matvec
    copy("vh1"); copy("vh2"); add(); copy("vh3"); add()
    copy("vh0"); swap(); sub()
    n_res = _n_boundary_residuals(n_out, range_spec)
    assert above[0] == 2 * n_res, "boundary_ext left %d limbs (expected %d)" % (above[0], 2 * n_res)
    prog += [OP("TOALT")] * (2 * n_res) + drop_n_prog(2 * len(ops)) + [OP("FROMALT")] * (2 * n_res)
    return prog
...
def _split_cells_loop_core(n):
    """Inner BEGIN/UNTIL: top is a blob of exactly 8*n bytes -> [cell_0..cell_{n-1}] (last on top).

    PCO-FRI-05 dens: count-down + OP_NOT exit (not count-up + NUM(n) NUMEQUAL). Same n pinned
    iterations and cell order; drops per-iter push of n. Dual e2b beta split is n=36 (chunked).
    Formal: SinkNotExit.not_exit_eq_numequal_zero on zero-counter."""
    if n <= 0:
        return []
    body = [NUM(8), OP("SPLIT"), OP("SWAP"), PUSH(b"\x00"), OP("CAT"), OP("BIN2NUM"), OP("SWAP")]
    return ([NUM(n), OP("TOALT"), OP("BEGIN")] + body
            + [OP("FROMALT"), OP("1SUB"), OP("DUP"), OP("TOALT"), OP("NOT"), OP("UNTIL"),
               OP("FROMALT"), OP("DROP"), OP("DROP")])


def split_cells_loop(n, prefix=0, chunk=32):
    ...
    OP-COST LEVER (chunk): for large n, CAT/SPLIT on a shrinking multi-kB rest is superlinear in cashvm/libauth.
    Process the blob in fixed-size chunks of `chunk` cells (default 32) so per-cell op stays near-linear —
    same stack result, ~constant redeem (one small loop body per chunk). chunk<=0 disables batching."""
    prog = []
    if prefix:
    ...
    if chunk and n > chunk:
        ...
        while remaining > 0:
            ...
            if remaining > take:
                # [cells..., blob] -> SPLIT front take*8 -> [chunk, rest]; rest to alt; cells from chunk; restore rest
                prog += [NUM(take * 8), OP("SPLIT"), OP("TOALT")]          # [cells..., chunk]; alt: rest
                prog += _split_cells_loop_core(take)
                prog += [OP("FROMALT")]                                    # [cells..., rest]
            else:
                prog += _split_cells_loop_core(take)
            remaining -= take
        return prog
    return prog + _split_cells_loop_core(n)


def drop_n_prog(n):
    """BYTE-LEVER: drop n top-of-stack items.

    Small n: OP_2DROP pairs (+ final DROP if odd) — half the redeem bytes of unrolled DROP, same effect.
    Large n (≥16): pinned BEGIN/UNTIL counter over OP_2DROP so redeem stays ~20B instead of n (or n/2) bytes.
    Iteration count is BAKED (never a witness); stack effect is identical to [OP("DROP")]*n."""
    if n <= 0:
        return []
    if n < 16:
        prog = [OP("2DROP")] * (n // 2)
    ...
    if n & 1:
        ...
        n -= 1
    half = n // 2
    # counter on alt; each iter 2DROP one pair; exit when counter hits 0.
    prog += [NUM(half), OP("TOALT"), OP("BEGIN"),
             OP("2DROP"),
             OP("FROMALT"), OP("1SUB"), OP("DUP"), OP("TOALT"),
             NUM(0), OP("NUMEQUAL"), OP("UNTIL"),
             OP("FROMALT"), OP("DROP")]
    return prog


def sink_top_n_under_block_prog(n_top, block_len):
    """BYTE-LEVER: stack [block (block_len items), n_top items on top] → [n_top items, block].

    Matches the gm_derive / fri_loop sink choreography: `NUM(block_len+n_top-1) ROLL` repeated
    `block_len` times (validated). Unrolled cost is O(block_len) redeem bytes; the pinned counter
    loop is ~20B. Iteration count is BAKED (never a witness).

    PCO-FRI-05 dens inject: exit via OP_NOT on the post-1SUB counter (zero → true) instead of
    `NUM(0) NUMEQUAL`. Same permutation; lower op-cost on E2b root/beta inject sinks (dual FRI
    dens cliff). Formal: FriStark.Optimizer.SinkNotExit.not_exit_eq_numequal_zero.

    Prefer bury_top_n_under_block_prog for E2b unlock-scale inject (large block_len): deep ROLL is
    O(block_len²) op via stackPushedBytes+=depth; bury is O(block_len) with small constants."""
    if n_top < 1 or block_len < 1:
        return []
    depth = block_len + n_top - 1
    count = block_len
    return [NUM(count), OP("TOALT"), OP("BEGIN"),
            NUM(depth), OP("ROLL"),
            OP("FROMALT"), OP("1SUB"), OP("DUP"), OP("TOALT"),
            OP("NOT"), OP("UNTIL"),
            OP("FROMALT"), OP("DROP")]
...
def ext_fold_stack_prog():
    """HP3.10 DEPLOYABLE GF(p^2) FRI fold (E1 fused butterfly).

    Nine stack operands (_FOLD_OPS): carried fv + sibling fw + beta + inv2 + i2x + twox.
    Computes folded = (fv+fw)*inv2 + beta*(fv-fw)*i2x over GF(p^2).

    Optimisations vs the first deployable form:
      - skip inv2*2==1 every butterfly (inv2 is verifier-pinned NUM(2^{-1}), not prover freedom)
      - materialize odd=(fv-fw)*i2x once; Karatsuba p00=odd0*b0, p11=odd1*b1 once; reuse for both limbs
      - retain i2x*twox==1 (fail-closed inverse hint)

    Leaves [folded0, folded1]; drops the nine operands. Needs field prelude (P on alt + MOD table).
    """
    prog = []
    above = [0]
    top_index = {n: i for i, n in enumerate(reversed(_FOLD_OPS))}  # top-relative depths into the 9-op window
    ...
    # Fail-closed: only the prover-supplied i2x must match twox.
    pick("i2x"); pick("twox"); mm(); num(1); neqv()

    # odd0 = (fv0-fw0)*i2x ; odd1 = (fv1-fw1)*i2x
    pick("fv0"); pick("fw0"); sm(); pick("i2x"); mm()
    pick("fv1"); pick("fw1"); sm(); pick("i2x"); mm()
    # p00 = odd0*b0 ; p11 = odd1*b1  (reuse for both limbs)
    prog.extend([NUM(1), OP("PICK")]); above[0] += 1
    pick("b0"); mm()
    prog.extend([NUM(1), OP("PICK")]); above[0] += 1
    pick("b1"); mm()
    # folded0 = (fv0+fw0)*inv2 + p00 + EXT_W*p11
    pick("fv0"); pick("fw0"); am(); pick("inv2"); mm()
    prog.extend([NUM(2), OP("PICK")]); above[0] += 1   # p00
    am()
    prog.extend([NUM(1), OP("PICK")]); above[0] += 1   # p11
    num(EXT_W); mm(); am()
    # folded1 = (fv1+fw1)*inv2 + (odd0+odd1)*(b0+b1) - p00 - p11
    pick("fv1"); pick("fw1"); am(); pick("inv2"); mm()
    prog.extend([NUM(5), OP("PICK")]); above[0] += 1   # odd0
    prog.extend([NUM(5), OP("PICK")]); above[0] += 1   # odd1
    am()
    pick("b0"); pick("b1"); am(); mm()
    prog.extend([NUM(4), OP("PICK")]); above[0] += 1   # p00
    sm()
    prog.extend([NUM(3), OP("PICK")]); above[0] += 1   # p11
    sm()
    am()
    # stack above ops: odd0, odd1, p00, p11, folded0, folded1
    assert above[0] == 6, "ext_fold_stack left %d intermediates (expected 6 before cleanup)" % above[0]
    prog += [OP("TOALT"), OP("TOALT")] + [OP("DROP")] * 4 + [OP("FROMALT"), OP("FROMALT")]
...
def merkle_verify_runtime_prog(trunc=0):
    ...
    trunc>0: N3 truncated-hash tree (keep first trunc bytes of each SHA256)."""
    tr = [NUM(trunc), OP("SPLIT"), OP("DROP")] if trunc else []
    step = [OP("FROMALT"), OP("DUP"), NUM(2), OP("MOD"),                # bit = k mod 2
            OP("SWAP"), NUM(2), OP("DIV"), OP("TOALT"),                 # k //= 2 back to alt
            OP("NOT"), OP("IF"), OP("SWAP"), OP("ENDIF"), OP("CAT"), OP("SHA256")] + tr
    dec = [OP("FROMALT"), OP("FROMALT"), OP("1SUB"), OP("DUP"), OP("TOALT"),   # dc -= 1
           OP("SWAP"), OP("TOALT"), NUM(0), OP("NUMEQUAL")]                    # restore [dc,k]; exit dc==0
    return ([OP("TOALT"), OP("TOALT"), OP("SHA256")] + tr + [OP("BEGIN")] + step + dec
...
def fri_coset_open_runtime_prog(s):
    ...
    try:
        ...
        _tr = 0 if _mhb >= 32 else _mhb
    except Exception:
        _tr = 0
    return ([NUM(2), OP("PICK"), OP("TOALT")]                           # stash coset_preimage on alt
            + merkle_verify_runtime_prog(_tr)                          # verify (runtime depth, N3 trunc)
...
def gm_derive_sink_prog(oN):
    ...
    p += sink_top_n_under_block_prog(4, 32)                             # sink g0..g3 below the 32-item block
    return p
...
def toalt_n_prog(n):
    """Park top n main-stack items on alt (counter-on-top loop for large n).

    Entry: [x0..x_{n-1}] (x_{n-1} top). Exit: main empty of those; alt has them
    (top = x0, so a matching fromalt_n_prog restores original order).
    Baked n — never a witness. Small n unrolled for op/byte balance."""
    if n <= 0:
        return []
    if n <= 15:
        return [OP("TOALT")] * n
    # [items, n] → loop: SWAP TOALT 1SUB until 0
    return [NUM(n), OP("BEGIN"),
            OP("SWAP"), OP("TOALT"), OP("1SUB"),
            OP("DUP"), NUM(0), OP("NUMEQUAL"), OP("UNTIL"),
            OP("DROP")]


def bury_top_n_under_block_prog(n_top, block_len, unroll_limit=96):
    """Op-cheap bury: same stack effect as sink_top_n_under_block_prog.

    Peel each block item from under the n_top group with fixed-depth ROLL(n_top) + TOALT
    (depth cost O(n_top), not O(block_len)), then FROMALT restore. Unrolled for block_len
    ≤ unroll_limit (redeem O(block_len); dual E2b headroom keeps ss≤10k at 96). Above the
    limit, fall back to sink ROLL loop (O(1) redeem, higher op).

    n_top==1 peels via SWAP (ROLL 1 ≡ SWAP) for one fewer push per item.
    Validated order-identical to sink_top_n_under_block_prog on cashvm."""
    if n_top < 1 or block_len < 1:
        return []
    if block_len > unroll_limit:
        return sink_top_n_under_block_prog(n_top, block_len)
    prog = []
    if n_top == 1:
        # [b0..b{L-1}, x]: (SWAP TOALT)×L then FROMALT×L → [x, b0..b{L-1}]
        for _ in range(block_len):
            prog += [OP("SWAP"), OP("TOALT")]
    else:
        # [block, tops]: ROLL(n_top) brings topmost b above tops; TOALT parks it.
        for _ in range(block_len):
            prog += [NUM(n_top), OP("ROLL"), OP("TOALT")]
    for _ in range(block_len):
        prog.append(OP("FROMALT"))
    return prog


def um_from_gm_prog(off):
    """HP11 L2 (op-cost lever): convert g0..g3 (below the 32-item param block) into
    u_m = (off * g_m)^pe. Algebra: (off*g_m)^(pe*2^f) = u_m^(2^f) so butterflies only
    square f times. Net −3 modexp/round vs L1-only (7 exp2 → 4 pe + squarings).

    Byte-compact (no TOALT×32): compute all four u_m onto the top, then DROP the four g's
    that sit just under the block by rolling the block+u's down. Uses sink only.

    Precondition (bottom→top): [g0,g1,g2,g3, stride, pe, betas(6), i2x(7), base, coset(16)].
    Leaves:                [u0,u1,u2,u3, stride, pe, betas(6), i2x(7), base, coset(16)].
    """
    p = []
    # Build u0..u3 on top. After m results, g_m is always at depth 35 (see L1 pick invariant).
    for m in range(4):
        p += [NUM(35), OP("PICK")]                                     # g_m
        p += [NUM(off % P), OP("MUL")] + _MODT                         # off * g_m
        p += [NUM(30 + m + 1), OP("PICK")]                             # pe (depth shifts with m u's + off*g)
        p += _modexp_invoke()                                          # u_m
    # bottom→top: g0,g1,g2,g3, block32, u0,u1,u2,u3
    # Sink 4 u's under (g+block)=36 → [u0..u3, g0..g3, block32]; DROP g's via ROLL.
    p += sink_top_n_under_block_prog(4, 36)
    for _ in range(4):
        p += [NUM(32), OP("ROLL"), OP("DROP")]
    # stack: [u0,u1,u2,u3, block32]
    return p


def _fri_twox_L2(Hh, b, m, f):
    """HP11 L2 twox (net +1): pick pre-derived u_m = (off*g_m)^pe, raise to 2^f by f squarings, *2.
    Same stack slot as L1's g_m (um_from_gm overwrote g→u in place). No modexp in the butterfly."""
    t = [NUM(Hh + b + 11 - m), OP("PICK")]                             # u_m
    for _ in range(f):
        t += [OP("DUP"), OP("MUL")] + _MODT                            # u_m := u_m^2
    t += [NUM(2), OP("MUL")] + _MODT                                   # twox = 2 * u_m^(2^f)
    return t
...
def fri_coset_fold_loop_prog(s, oN, off):
    """HP1.9-FOLD8 HP3.10 (+HP11 L1/L2): runtime-param coset fold (s=3).

    HP11 L1: gm_derive_sink derives g_m = oN^(base+m*stride) once (2 modexp) instead of per butterfly.
    HP11 L2: um_from_gm converts g_m → u_m = (off*g_m)^pe (4 modexp); each butterfly raises u_m to
    2^f by f squarings — eliminates 7 exp2-modexps/round (net −3 modexp/round vs L1 alone).

    Output is byte-identical to the baked fri_coset_fold_prog / _coset_fold oracle.
    Precondition (bottom→top): [stride, pe, betas(6), i2x(7), base, coset(16)].
    Leaves [u0..u3, param block, i2x_block, base, folded0, folded1] (u's dropped by caller).
    Needs field prelude + modexp DEFINE (caller installs)."""
    assert s == 3, "the HP3.10 loop body uses the uniform s=3 fold"
    prog = gm_derive_sink_prog(oN)                                       # HP11 L1: g0..g3 at bottom
    prog += um_from_gm_prog(off)                                         # HP11 L2: g→u = (off*g)^pe
    cnt = 1 << s
    K = (1 << s) - 1
    Hh = K + 1 + 2 * cnt
    inv2 = pow(2, P - 2, P)
    ...
    for f in range(s):
        ...
        for m in range(h):
            ...
            prog += _fri_twox_L2(Hh, b, m, f); b += 1                  # HP11 L2: u_m^(2^f)*2
            prog += [NUM((2 * cnt - 1) + b), OP("PICK")]; b += 1        # fv0
            prog += [NUM((2 * cnt - 2) + b), OP("PICK")]; b += 1        # fv1
            # BYTE-LEVER: INVOKE DEFINE'd ext_fold (0x2e) — installed by fri_loop_defines / caller.
            prog += [PUSH(_EXT_FOLD_FID), OP("INVOKE")]                # -> [folded0, folded1]
            Hh += 2; oi += 1
        nres = 2 * h; ncur = 2 * cnt
        prog += [OP("TOALT")] * nres + drop_n_prog(ncur) + [OP("FROMALT")] * nres
        Hh = K + 1 + nres
        cnt = h
    return prog
...
def fri_loop_body_prog(oN, off):
    ...
    E2a: stride/pe are carried-only (no stride_w/pe_w witness copy). Entry (bottom->top):
    [<later>, betas(6), i2x(7), base, root, sibs, preimage, k, d, ci, stride, pe, f0, f1].
    base == ci % stride remains the FRI-index consistency guard (verify:415).
    """
    p = [OP("TOALT")] * 5                                              # stash carried to alt (field-op-free)
    p += fri_coset_open_runtime_prog(3)                              # open (runtime depth) -> [.., coset cells]
    p += [OP("FROMALT")] * 5                                          # restore carried -> [.., coset, carried(5)]
    # After open (top-relative): f1=0,f0=1,pe=2,str=3,ci=4, coset=5..20, base=21
    # (base depth unchanged by E2a: sw/pw were below base.)
    p += [NUM(4), OP("PICK"), NUM(4), OP("PICK"), OP("MOD"),          # ci % stride
    ...
    # sink next3 under (betas+i2x+base+coset+carried) = 35; top becomes f1..ci
    p += sink_top_n_under_block_prog(3, 35)
    # E2a: drop f1,f0,ci; keep carried str/pe as fold param prefix (replaces removed stride_w/pe_w).
    # After sink top-rel: f1,f0,pe,str,ci,coset...,base,i2x,betas | next3 below.
    p += [OP("DROP"), OP("DROP")]                                     # drop f1,f0 -> pe,str,ci,...
    p += [OP("TOALT"), OP("TOALT"), OP("DROP")]                       # park pe,str; drop ci
    p += [OP("FROMALT"), OP("FROMALT")]                               # restore str,pe on top (FROMALT reverses)
    # top: pe,str,coset...,base,i2x,betas,next3 — need [next3,str,pe,betas,i2x,base,coset]
    # After FROMALT: str then pe on top (alt had pe then str). Want str,pe under the 30-item mid block.
    p += sink_top_n_under_block_prog(2, 30)                           # -> [next3, str, pe, betas, i2x, base, coset]
    p += fri_coset_fold_loop_prog(3, oN, off)                        # fold on 32-item param block
    p += [OP("TOALT")] * 2 + drop_n_prog(20) + [OP("FROMALT")] * 2  # cleanup (HP11 L1: +4 g_m) -> next carried
    return p


def fri_loop_round_witness(stride, pe, betas_round, base, coset, path, root, i2x_list,
                           include_root=True, include_betas=True):
    """Push-only per-round witness for fri_loop_body_prog (below the carried, bottom->top):
    [betas(6)?, i2x(7), base, root?, sib_{d-1}..sib_0, preimage, k=base, d].

    E2a: stride/pe are NOT witnessed — they are carried structural state only (fri_loop_body_prog).
    Callers may still pass stride/pe for API stability; they are ignored in the unlock bytes.

    E2b: include_root=False omits PUSH(root) (~MERKLE_HASH_BYTES+1 per round). Sound path
    re-inserts blob fri_roots[r] onto the stack in redeem before folds (single-source, no unlock bytes).

    E2b-betas: include_betas=False omits 6 beta limbs; sound path inserts FS betas from blob.
    """
    preimage = b"\x00" * 16 + b"".join(enc8(v[0] % P) + enc8(v[1] % P) for v in coset)
    sibs = [bytes.fromhex(sh) for sh, _ in path]
    w = []
    ...
    if include_root:
        w += [PUSH(root)]
    w += [PUSH(sib) for sib in reversed(sibs)] + [PUSH(preimage), NUM(base), NUM(len(sibs))]
    return w
...
_EXT_FOLD_FID = b"\x2e"  # GF(p^2) FRI butterfly fold body (ext_fold_stack_prog)
...
def fri_loop_defines(oN, off):
    ...
    BYTE-LEVER: also DEFINE ext_fold (0x2e) once so fri_coset_fold_loop INVOKEs the butterfly instead of
    inlining ~209B × 7 butterflies."""
    return (modexp_define()
            + [PUSH(_EXT_FOLD_FID), DEFINE(ext_fold_stack_prog())]
            + [PUSH(_LOOPBODY_FID), DEFINE(fri_loop_body_prog(oN, off))])
...
def fs_input_prog(N, n_ext_alphas, n_queries, n_out, n_r, fold_step, grind_b, deep=False, n_terms=0, oT=None):
    ...
    try:
        from native_ct_air_config import MERKLE_HASH_BYTES as _RH
    except Exception:
        _RH = 32
    tp_off = 16 + 8 * n_out                                    # tp_root offset in the blob
    cr_off = tp_off + _RH + _RH * n_r                          # HP11.1 comp_root after tp + fri_roots
    zg_off = cr_off + _RH                                      # HP11.4 committed z*g (bound == oT*z; not absorbed)
    mask_off = zg_off + 16                                     # HP11.5 mask block (16 bytes = 2 limbs per DEEP term)
    prefix_len = 8 + 8 + 8 * n_out + _RH * (1 + n_r) + ((_RH + 16 + 16 * n_terms) if deep else 0)
    ...
    prog += _absorb_from_blob(tp_off, _RH)                     # tp_root FIRST (MERKLE_HASH_BYTES)
    ...
    if deep:                                                 # HP11 DEEP transcript segment (mirrors fs_transcript_prog)
        prog += _absorb_from_blob(cr_off, _RH)                           # HP11.2 comp_root AFTER the AIR-alphas
    ...
    size = N; ri = 0
    while True:
        prog += _absorb_from_blob(tp_off + _RH + _RH * ri, _RH); ri += 1   # fri_root per committed round
...
def fri_loop_s3_redeem_e2b_sw(s3_rounds, root_tokens_per_round, beta_tokens_per_round,
                              defines_installed=True, oN=None, off=None):
    """PCO-FRI-05 per-INVOKE small-window inject (E2b dens unlock).

    Unlock omits roots+betas. Before each loop-body INVOKE, caller-supplied tokens leave the
    round's root (1 item) and betas (6 limbs) on top of [round_ws_no_root_beta, carried(5)];
    we sink them into the round window only:

      root  under carried+window_top = d+8   (window_top = sibs+preimage+k+d = d+3)
      betas under carried+full_window = d+17 (window = i2x+base+root+sibs+pre+k+d = d+12)

    Path depths on secure tip are 18..3 so max sink is ~35 — not unlock-scale hundreds.
    root_tokens_per_round / beta_tokens_per_round: lists of token lists, len == len(s3_rounds).
    """
    defs = [] if defines_installed else fri_loop_defines(oN, off)
    assert len(s3_rounds) == len(root_tokens_per_round) == len(beta_tokens_per_round)
    prog = list(defs)
    for r, rt, bt in zip(s3_rounds, root_tokens_per_round, beta_tokens_per_round):
        d = len(r["path"])
        # Stack: [..., i2x(7), base, sibs(d), preimage, k, d, ci, str, pe, f0, f1]
        prog += list(rt)
        prog += sink_top_n_under_block_prog(1, d + 8)
        prog += list(bt)
        prog += sink_top_n_under_block_prog(6, d + 17)
        prog += [PUSH(_LOOPBODY_FID), OP("INVOKE")]
    return prog
...
def _fri_loop_chain_redeem(rounds, k, final, oN, off, fri_root_last, defines_installed=False,
    ...
    fri_loop_defines body + carried-build + n INVOKE + juggle + final-bind + optional HP11 F3 final-ROOT-bind.

    bind_final_root=False (E3a): omit the PUSH(final leaves)+fri_final_root_bind_prog. Use when the caller
    amortizes one merkle(final)==fri_root_last per FRI *input* (e.g. gq=2 multi-query redeem) after all
    per-query fri_final_bind steps, or when an outer frl_bind already pairs with a single root-bind.

    include_frl: whether this query's unlock carries fri_root_last. Default matches bind_final_root.
    When False (E3a non-last query), unlock omits the ~26B frl push and redeem skips the matching DROP.
    """
    ...
    prog += fri_final_bind_prog(L)                              # folded'==final[ci'%L]; stack now [fri_root_last] if present
    if bind_final_root:
        # HP11 F3: merkle(final)==fri_root_last (fail-closed). fri_root_last at unlock bottom.
        assert include_frl, "bind_final_root requires include_frl"
        for f in final:
            prog += [PUSH(enc8(f[0] % P) + enc8(f[1] % P))]
        prog += fri_final_root_bind_prog(L)
    elif include_frl:
        # E3a multi-query (legacy): unlock still had frl but no root-bind — drop it.
        prog += [OP("DROP")]
    # else: E3a + include_frl=False — no frl on stack (scriptSig win ~26B per non-last query)
    return prog + [NUM(1)]                                      # [1] on accept


def fri_loop_round_witness_prod_e2b(i2x_list, base, path, preimage_coset, k):
    """PRODUCT PATCH (2026-08-06): kind-independent E2b round witness — [i2x(7), base, sibs, preimage, k, d].
    No sw/pw (structural, baked by the redeem), no betas/root (read from the blob by the redeem inject)."""
    sibs = [bytes.fromhex(sh) for sh, _ in path]
    w = [NUM(x % P) for x in i2x_list] + [NUM(base % P)]
    w += [PUSH(sib) for sib in reversed(sibs)] + [PUSH(preimage_coset), NUM(base), NUM(len(sibs))]
    return w


def _fri_loop_chain_unlock_prod_e2b(rounds, fri_root_last, final_pre):
    """PRODUCT PATCH: unlock = [final_pre, frl, prod-e2b round witnesses] (no roots/betas/sw/pw)."""
    s3 = [r for r in rounds if r["s"] == 3]
    w = list(final_pre) + [PUSH(fri_root_last)]
    for r in reversed(s3):
        preimage = b"\x00" * 16 + b"".join(enc8(v[0] % P) + enc8(v[1] % P) for v in r["coset"])
        w += fri_loop_round_witness_prod_e2b(r["i2x"], r["base"], r["path"], preimage, r["base"])
    return w


def _fri_loop_chain_unlock(rounds, comp, fri_root_last, include_frl=True, include_root=True, include_betas=True):
    """HP3.10(vi) UNLOCK-SPLIT witness (scriptSig): [fri_root_last?, <round witnesses last-round-deepest>,
    ...
    include_frl=False: omit fri_root_last (E3a non-last query in a multi-query FRI input — root-bind is
    amortized onto the last query only; the outer frl_bind still pins the last query's frl to the blob).

    include_root=False (E2b): omit per-round Merkle roots from unlock; sound redeem inserts blob fri_roots.
    include_betas=False (E2b-betas): omit FS beta limbs; sound redeem inserts from blob fs_out.
    """
    s3 = [r for r in rounds if r["s"] == 3]
    w = [PUSH(fri_root_last)] if include_frl else []            # HP11 F3: fri_roots[-1] at the unlock bottom
    for r in reversed(s3):
        w += fri_loop_round_witness(r["stride"], 1 << r["li0"], r["betas"], r["base"], r["coset"],
                                    r["path"], bytes.fromhex(r["root"]), r["i2x"],
                                    include_root=include_root, include_betas=include_betas)
...
def fri_final_root_bind_prog(n_final, trunc=None):
    """HP7.4c: bind the prover's clear final FRI layer to its COMMITMENT fri_roots[-1] (== merkle(final),
    stark.py Hf-tree). fri_roots[-1] is FS-absorbed (verify:340/347) but the clear proof["final"] was NEVER
    ...
    leaf_j = Hf(0x00*16 || enc8(v_j0) || enc8(v_j1)); adjacent pairs are folded Hf(left||right) up the tree;
    require the root == the FS-single-sourced fri_roots[-1] (HP9.3). `final` stays a WITNESS but is now
    root-bound. n_final is a power of 2 so the commitment tree is a perfect binary tree.

    trunc: N3 Merkle width. None = read MERKLE_HASH_BYTES (0 when full SHA256). trunc>0 keeps first n
    bytes after each SHA256 so nodes/root match Hf.

    Stack precondition (bottom->top): [fri_root_last(MERKLE_HASH_BYTES), pre_0, ..., pre_{L-1}] where pre_j =
    enc8(final[j][0]) || enc8(final[j][1]). Leaves [] -- EQUALVERIFY fail-closed on mismatch."""
    if n_final < 1 or (n_final & (n_final - 1)) != 0:
        raise ValueError("fri_final_root_bind_prog: n_final must be a power of 2, got %d" % n_final)
    if trunc is None:
        try:
            from native_ct_air_config import MERKLE_HASH_BYTES as _mhb
            trunc = 0 if _mhb >= 32 else _mhb
        except Exception:
            trunc = 0
    tr = [NUM(trunc), OP("SPLIT"), OP("DROP")] if trunc else []
    salt = b"\x00" * 16
    ...
    for j in range(n_final):
        prog += [OP("FROMALT"), PUSH(salt), OP("SWAP"), OP("CAT"), OP("SHA256")] + tr  # leaf_j = Hf(salt||pre_j)
        t = j
        while t & 1:                                           # combines_after(j) = trailing 1-bits of j
            prog += [OP("CAT"), OP("SHA256")] + tr             # Hf(left_deeper || right_top)
            t >>= 1
...
def ext_zeval_hint_check_prog(log2T, last, hd_vals):
    ...
    # BYTE-LEVER: DEFINE shared inv-check body; unrolled r still needs per-row Hd + PICK indices.
    # FID 0x2b (not 0x2a): 0x2a is COMPOSE_MAC_T_LAZY — LEVER-6 fuses last-trans+final in one redeem.
    _INV = b"\x2b"
    prog.extend([PUSH(_INV), DEFINE(ext_inv_check_prog())])
    for r, hd in enumerate(hd_vals):
        ...
        prog.extend([PUSH(_INV), OP("INVOKE")]); sp[0] -= 4   # bound_invX[r]*d == (1,0)
...
def comp_split_trans_part_prog(jobs, part_n_res, m_ext, diag, minv_cap_rows, sbox_looped=False, state_looped=False,
    ...
    if state_looped and has_state:
        ...
        # BYTE-LEVER: capacity MAD (0x2c) for M4 matvec — same body chain_ext uses.
        prog += [PUSH(_CAP_MAD_FID), DEFINE(_cap_mad_body())]
    ...
    def mad_into(s, name):
        """acc += s * block[name] via DEFINE 0x2c (acc already on top as 2 limbs)."""
        ei = aidx[name]
        prog.extend([NUM(sp[0] - 1 - 2 * ei), OP("PICK")]); sp[0] += 1
        prog.extend([NUM(sp[0] - 1 - (2 * ei + 1)), OP("PICK")]); sp[0] += 1
        prog.append(NUM(s % P)); sp[0] += 1
        prog.extend([PUSH(_CAP_MAD_FID), OP("INVOKE")]); sp[0] -= 3
        ...
        if state_looped:                                         # ef = M_EXT.y_full via the M4-block factorization
            for blk in range(0, W, 4):                           # M4 per 4-block: mm[blk+r] = sum_k M4[r][k]*yf[blk+k]
                for r in range(4):
                    prog.extend([NUM(0), NUM(0)]); sp[0] += 2   # acc = 0
                    for kk in range(4):
                        mad_into(_M4_EXT[r][kk], "yf%d" % (blk + kk))
            for l in range(4):                                   # stored[l] = mm[l] + mm[4+l] + mm[8+l] (block sums)
    ...
    if carry_bound:                                            # bind computed acc_out == carry_out_preimage @ 0
        ...
        return prog + [OP("TOALT")] + drop_n_prog(2 * total_below) + [OP("FROMALT")]   # -> [1]
    return prog + [OP("TOALT"), OP("TOALT")] + drop_n_prog(2 * total_below) + [OP("FROMALT"), OP("FROMALT")]
...
def comp_split_final_part_prog(minv0_row, n_out, range_spec, root, nf, cm_out, n_b,
                               carry_bound=False, comp_bound=False, mad_defined=False,
                               public_from_blob=False):
    ...
    for i in range(4):
        pk_at(e_vh + i)
    prog += boundary_ext_residuals_prog(minv0_row, n_out, range_spec, root, nf, cm_out,
                                        mad_defined=mad_defined, public_from_blob=public_from_blob)
