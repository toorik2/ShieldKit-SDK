...
test('Q-08 accepts only exact absolute one-shot inputs', async () => {
  ...
  assert.throws(() => parseV2Q08Arguments([
    '--output-dir', 'relative', '--descriptor', '/a', '--final-manifest', '/b', '--profile-core', '/c', '--release-root', 'final-chipnet', '--command-plan', '/e', '--funding-checkpoint', '/f', '--host-identity', '/g', '--host-role', 'clean-host-a', '--host-signing-key', '/h', '--expected-commit', '0'.repeat(40), '--expected-tree', '1'.repeat(40),
  ]), /absolute normalized/u);
  assert.throws(() => parseV2Q08Arguments([
    '--output-dir', '/a', '--descriptor', '/b', '--final-manifest', '/c', '--profile-core', '/d', '--trusted-signers', '/e', '--command-plan', '/f', '--funding-checkpoint', '/g', '--host-identity', '/h', '--host-role', 'clean-host-a', '--host-signing-key', '/i', '--expected-commit', '0'.repeat(40), '--expected-tree', '1'.repeat(40),
...
test('Q-08 bounded command doubles are test-only and cannot emit a host transcript', async (t) => {
  ...
  const result = await runV2Q08CleanMachineQualification({
    ...
    profileCorePath, releaseRootId: 'test-only-root', commandPlanPath: planPath, fundingCheckpointPath: fundingPath,
