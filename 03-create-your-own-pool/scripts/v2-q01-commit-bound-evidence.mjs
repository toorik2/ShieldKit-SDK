...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) { try { const args = parseV2Q01CommitBoundArguments(process.argv.slice(2)); const result = args.mode === 'verify' ? verifyV2Q01CommitBoundBundle(args.bundlePath) : await runV2Q01CommitBoundEvidence({ outputDirectory: args.outputDirectory }); process.stdout.write(`${canonicalJson(result)}\n`); } catch (error) { process.stderr.write(`${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 1; } }
