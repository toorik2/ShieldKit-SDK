...
function poolCreatePerformance() {
  ...
  return {
    schema: 'shieldkit-v2-beta-live-pool-create-performance-v2',
...
test('rejects duplicate actions, telemetry/runtime tampering, bad percentiles, and stale manifest facts', () => {
  ...
  const mismatchedReceipt = poolCreatePerformance(); mismatchedReceipt.receiptSha256 = H('f');
  assert.throws(() => createV2BetaLiveEvidenceBundleManifest({ semanticBytes: canonicalBytes(semantic()), performanceBytes: canonicalBytes(performance()), poolCreatePerformanceBytes: canonicalBytes(mismatchedReceipt) }, facts()), rejects('BUNDLE_POOL_CREATE_PERFORMANCE_INVALID'));
