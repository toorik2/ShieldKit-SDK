...
Optional demo: 02-use-chipnet-demo-pool/   (live Chipnet instance)
  ...
  pool add-funding --funding-utxo <64-lowercase-hex-txid:vout> [--data-home <absolute-directory>] [--human|--json]  # register an already-owned live UTXO; never sends
  ...
  (V2 beta Chipnet: one user-funded invocation; wallet must be a canonical owner-private 0600 JSON file and every funding UTXO must be unspent, tokenless P2PKH owned by that retained wallet. pool add-funding authenticates and registers one exact existing UTXO; it never scans or sends. A crash-resume is explicit and cannot substitute new funding. No sponsor, faucet, RPC account, API key, custom endpoint, or SSH. The sole action send goes to one pinned public provider; exact zero-conf completion requires visibility through at least two independent pinned providers, and action admissions use two non-broadcasting witnesses—never confirmation or mining. Capacity 100000; explicitly unqualified. Public providers can observe the client IP and queried transactions/outpoints. After a ShieldKit source upgrade, run pool refresh-runtime locally before actions; it never proves or contacts a network provider.)
