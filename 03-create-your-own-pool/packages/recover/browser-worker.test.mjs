...
test('Chromium module worker executes the bundled portable entrypoint with WebCrypto CSPRNG', async () => {
  await build({ entryPoints: [new URL('./recovery.mjs', import.meta.url).pathname], outfile: bundle.pathname, bundle: true, format: 'esm', platform: 'browser', target: 'es2022', legalComments: 'none' });
  ...
  try {
    ...
    const { stdout } = await execFileAsync('/usr/bin/chromium', ['--headless=new', '--no-sandbox', '--disable-gpu', '--virtual-time-budget=10000', '--dump-dom', `http://127.0.0.1:${port}/`], { maxBuffer: 1024 * 1024 });
