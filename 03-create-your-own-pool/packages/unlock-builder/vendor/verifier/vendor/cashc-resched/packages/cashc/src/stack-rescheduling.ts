...
export function applyStackRescheduling(
  ...
  // the layout search needs main to be re-writable (it may have to re-stage arguments)
  const layouts = optimised.logs.length === 0;
  ...
  return {
    result: { ...optimised, script, locationData, requires, sourceTags },
    frames: newFrames,
  };
}
