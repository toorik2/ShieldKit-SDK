...
// token id for a value-producing ref (const -> null: always freshly pushed)
...
export function scheduleBlock(block, arity, opts = {}) {
  ...
  const materialize = (ref) => {
    if (ref.k === 'const') { for (const b of encConst(ref)) emit(b); S.push({ tok: null, t: true }); return; }
    const tok = tokenOf(ref);
    const remaining = use.get(tok);
    const d = depthOfHome(tok);
    if (d < 0) throw new Error('scheduler: home not found for ' + tok);
    if (remaining === 1) {
  ...
  if (readyOrder) {
    while (remaining.size) {
      ...
      rs.sort((a, b) => operandDepthScore(a) - operandDepthScore(b) || a.id - b.id);
      const node = rs[0];
      emitOne(node);
  ...
  for (let e = L; e < exit.length; e++) {
    const ref = exit[e];
    if (ref.k === 'const') { for (const b of encConst(ref)) emit(b); S.push({ tok: null }); continue; }
    const tok = tokenOf(ref);
    const d = depthOfHome(tok);
  ...
  function materializeCopyToTop(ref) {
    if (ref.k === 'const') { for (const b of encConst(ref)) emit(b); S.push({ tok: null, t: true }); use && 0; return; }
    const tok = tokenOf(ref);
    const remaining = use.get(tok) || 0;
    const d = depthOfHome(tok);
    if (d < 0) throw new Error('scheduler(alt): home not found for ' + tok);
