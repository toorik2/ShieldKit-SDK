...
import { loadPinnedV2DirectGroth16AdapterResult, sha256Bytes } from '../src/c7/v2-direct-groth16-adapter-input.mjs';
...
test('pinned complete material is coordinate-equivalent to verifier.cash static vector and verifies its real Groth16 equation', async () => {
  ...
  try {
    ...
    const loaded = await loadPinnedV2DirectGroth16AdapterResult({ path: filename, sha256: await digest(filename) });
...
test('pinned import rejects hash drift, duplicate keys, legacy PF7 schema, and unsupported V2 Direct arity', async () => {
  const directory = await mkdtemp(path.join(here, '.tmp-shield-adapter-'));
  try {
    ...
    await assert.rejects(() => loadPinnedV2DirectGroth16AdapterResult({ path: filename, sha256: '0'.repeat(64) }), /SHA-256 mismatch/);
    const legacy = adapterResultFor(vec); legacy.schema = 'shield.cash/snarkjs-groth16-pf7-adapter/v1';
    await writeFile(filename, `${JSON.stringify(legacy)}\n`);
    const legacyHash = await digest(filename);
    await assert.rejects(() => loadPinnedV2DirectGroth16AdapterResult({ path: filename, sha256: legacyHash }), /unsupported schema/);
    await writeFile(filename, '{"schema":"x","schema":"x"}');
    const duplicateHash = await digest(filename);
    await assert.rejects(() => loadPinnedV2DirectGroth16AdapterResult({ path: filename, sha256: duplicateHash }), /duplicate JSON object name/);
    ...
    await assert.rejects(() => loadPinnedV2DirectGroth16AdapterResult({ path: filename, sha256: malformedHash }), /requires exactly two public signals/);
  } finally { await rm(directory, { recursive: true, force: true }); }
});

test('V2 Direct rejects canonical snarkjs IC0 infinity without identity substitution', async () => {
  const directory = await mkdtemp(path.join(here, '.tmp-shield-adapter-'));
  try {
    const vec = JSON.parse(await readFile(vectorsPath, 'utf8')); const filename = path.join(directory, 'adapter-result.json');
    ...
    await assert.rejects(() => loadPinnedV2DirectGroth16AdapterResult({ path: filename, sha256: infinityHash }), /canonical infinity; V2 Direct requires finite IC points/);
  } finally { await rm(directory, { recursive: true, force: true }); }
});
