...
async function fixture(t, {
  ...
  await writeSource(
    root,
    '03-create-your-own-pool/circuits/v2-direct/a.circom',
    render(aIncludes),
  );
  await writeSource(
    root,
    '03-create-your-own-pool/circuits/v2-direct/b.circom',
    render(bIncludes),
  );
...
test('rejects noncanonical, incomplete, and unreachable manifest graphs', async (t) => {
  ...
  withUnreachableSource.sources.push({
    bytes: 1,
    includes: [],
    path: '03-create-your-own-pool/circuits/v2-direct/unreachable.circom',
    sha256: '0'.repeat(64),
  });
...
test('rejects symlinked and hardlinked relation sources', async (t) => {
  ...
  await link(
    ...
    path.join(
      hardlinkRoot,
      '03-create-your-own-pool/circuits/v2-direct/a-copy.circom',
    ),
  );
  await assert.rejects(
