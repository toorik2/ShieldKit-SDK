...
import { LocalProverProfileError, parseLocalProverProfileBinding } from './local-prover.mjs';
...
test('local prover binding requires an absolute bundle and all immutable coordinates', () => {
  ...
  const mainnet = valid(); mainnet.expected.network = 'mainnet';
  assert.equal(parseLocalProverProfileBinding(mainnet).expected.network, 'mainnet');
});
