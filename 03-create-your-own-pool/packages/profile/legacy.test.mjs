...
test('legacy create-pool and chain E2E refuse before filesystem or network effects', async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), 'shieldkit-legacy-scripts-'));
  t.after(() => rm(root, { recursive: true, force: true }));

  await expectQuarantinedScript(
    '03-create-your-own-pool/scripts/create-pool.mjs',
    ['--out', path.join(root, 'pool')],
    path.join(root, 'pool'),
  );
  await expectQuarantinedScript(
    '03-create-your-own-pool/scripts/chain-e2e-chipnet.mjs',
    ['--out-dir', path.join(root, 'chain-e2e')],
    path.join(root, 'chain-e2e'),
  );
});
  ...
  await assert.rejects(
    execFileAsync(
      ...
      [
        path.join(
          repositoryRoot,
          '03-create-your-own-pool/scripts/shieldkit.mjs',
        ),
        'init',
        '--config',
