...
import { createDesktopComposition, WalletSdkError } from '../kit/desktop.mjs';
...
const binding = (value, label) => {
  ...
  if (value.expectedProfile.network !== 'chipnet' && value.expectedProfile.network !== 'mainnet') {
    fail(`${label}.expectedProfile.network must be chipnet or mainnet`);
  }
...
export async function runProfileReplacementDrill(input) {
  ...
  try {
    [leftSdk, rightSdk] = await Promise.all([
      createDesktopComposition({ bundleDirectory: leftInput.bundleDirectory, expectedProfile: expectedFor(left) }),
      createDesktopComposition({ bundleDirectory: rightInput.bundleDirectory, expectedProfile: expectedFor(right) }),
    ]);
  } catch (error) {
  ...
  try {
    await createDesktopComposition({ bundleDirectory: leftInput.bundleDirectory, expectedProfile: expectedFor(right) });
