...
import { PROVER_ARTIFACT_BUDGET_BYTES, ProverArtifactBudgetError, budgetVerdict, canonicalJson, packageProverArtifacts } from './budget.mjs';
...
test('CLI uses duplicate-safe manifest parsing, relative paths, and canonical success output', async (t) => {
  ...
  const { stdout, stderr } = await execFileAsync(process.execPath, ['budget-cli.mjs', '--input', manifest], { cwd: path.dirname(new URL(import.meta.url).pathname), env: {} });
  ...
  await assert.rejects(
    () => execFileAsync(process.execPath, ['budget-cli.mjs', '--input', duplicate], { cwd: path.dirname(new URL(import.meta.url).pathname), env: {} }),
