...
const lockHash = (lock) => bytesToHex(hash256(lock));
...
export function extractRawSettlementHistory(value) {
  exactKeys(value, 'raw settlement history input', ['instanceId', 'profileId', 'rawTransactions', 'stateLockHash', 'stateLockingBytecode', 'stateNftCategory']);
  const expected = { profileId: hex32(value.profileId, 'profileId'), instanceId: hex32(value.instanceId, 'instanceId'), stateNftCategory: hex32(value.stateNftCategory, 'stateNftCategory'), stateLockingBytecode: raw(value.stateLockingBytecode, 'stateLockingBytecode') };
  if (hex32(value.stateLockHash, 'stateLockHash') !== lockHash(expected.stateLockingBytecode)) fail('STATE_LOCK_HASH', 'stateLockHash does not authenticate stateLockingBytecode');
