...
test('Android SDK library source compiles on host JDK and has no network permission', async () => {
  const result = await verifyAndroidLibrary();
  assert.equal(result.schema, 'shield.cash/android-library-host-check/v1');
  assert.match(result.javac.version, /javac/); assert.ok(result.classFiles.length >= 6);
});
