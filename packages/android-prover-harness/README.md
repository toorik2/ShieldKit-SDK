...
This is a fail-closed, physical-Android/Termux execution harness for Groth16
artifacts loaded from an authenticated immutable verifier profile bundle. It
requires the exact Chipnet/profile/instance binding, then derives its zkey and
verification key from that bundle. It only accepts USB `adb`, direct regular files
...
The required release-package fixture, evidence fields, and explicit current
hardware boundary are in [REAL_DEVICE_GATE.md](REAL_DEVICE_GATE.md).
