/-
  ...
  recompiler project. It REUSES the real project defs — `LeanBCH.Opt.removeNth` (Basic.lean:28,
  the SAME order-preserving deletion `Op.ROLL` uses) and `LeanBCH.Opt.Floor.Stack` (`List Nat`,
  ...
  nested `LeanBCH.Opt.MovementFloor` namespace (parallel to `LeanBCH.Opt.Floor`).

  ★ STATUS: REFERENCE LEMMA — NOT LOAD-BEARING. `movement_lower_bound` bounds an ABSTRACT
  move-to-top counting machine, not the scheduler's real `emitMove` byte output; it has no
  downstream proof consumer, and the sharper `NlcsFloor.movement_lower_bound_nlcs` supersedes its
  disjoint-inverted-pairs bound. Kept as a proven combinatorial reference, NOT a live lower bound on
  any program the optimizer emits.
-/
...
/-- The counting stack: reuse the copy-floor's `LeanBCH.Opt.Floor.Stack` (`= List Nat`). -/
...
/-- The MOVEMENT primitive: bring the value at depth `d` to the TOP, deleting it from its old
    ...
    project's real `LeanBCH.Opt.removeNth` — the SAME deletion `Op.ROLL` uses (Basic.lean:73).
...
/-- On its success domain, `moveToTop` is the ROLL rewrite `v :: removeNth s d` (Basic.lean ROLL,
    literally the same `LeanBCH.Opt.removeNth`). -/
...
/-! ## BRIDGING the movement floor to the real scheduler (`LeanBCH.Opt.Scheduler`).
    ...
    ★ SUPERSEDED (but kept) by the TIGHT n-LCS floor: `LeanBCH.Opt.NlcsFloor.movement_lower_bound_nlcs`
